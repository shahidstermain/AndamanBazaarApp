...
interface WebhookEvent {
  type: 'PAYMENT_SUCCESS_WEBHOOK' | 'PAYMENT_FAILED_WEBHOOK' | 'PAYMENT_REFUND_WEBHOOK';
  timestamp: string;
  signature: string;
  data: {
...
class WebhookEventHandler {
  ...
  private validateTransition(currentState: string, eventType: string): StateTransition {
...
class StateRecovery {
  ...
  private async recoverPayment(payment: PaymentRecord): Promise<void> {
    ...
    if (payment.status === 'pending_payment' && stuckDuration > 30 * 60 * 1000) {
      // Payment stuck for > 30 minutes
      await this.handlePaymentTimeout(payment.id);
    } else if (payment.status === 'payment_success' && stuckDuration > 5 * 60 * 1000) {
...
class StateValidator {
  validateTransition(currentState: string, targetState: string, event: string): boolean {
    const validTransitions = {
      'initiated': ['pending_order_create'],
      'pending_order_create': ['order_created', 'order_failed'],
      'order_created': ['pending_payment', 'order_expired'],
      'pending_payment': ['payment_success', 'payment_failed', 'payment_cancelled', 'payment_timeout'],
...
class IntegrityChecker {
  async validatePaymentIntegrity(paymentId: string): Promise<IntegrityReport> {
    ...
    report.financialConsistent = await this.validateFinancialConsistency(payment);
...
class PaymentStateMachine {
  ...
  private validateTransition(event: PaymentEvent): void {
    ...
    if (!validator.validateTransition(this.currentState.status, event.targetState, event.type)) {
      throw new Error(`Invalid state transition: ${this.currentState.status} -> ${event.targetState}`);
    }
  }
}
```
