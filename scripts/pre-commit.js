  ...
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].length > config.limits.maxLineLength) {
        console.error(
          `❌ Line ${i + 1} in ${filePath} exceeds ${config.limits.maxLineLength} characters`,
        );
        return false;
      }
    }
  ...
  try {
    ...
    for (const func of allFunctions) {
      ...
      if (lines > config.limits.maxFunctionLength) {
        console.error(
          `❌ Function in ${filePath} exceeds ${config.limits.maxFunctionLength} lines`,
        );
        return false;
      }
    }
    ...
    for (let i = 0; i < lines.length; i++) {
      for (const pattern of securityPatterns) {
        if (pattern.test(lines[i])) {
          console.error(
            `❌ Security issue detected in ${filePath} at line ${i + 1}: ${lines[i].trim()}`,
          );
          return false;
        }
      }
...
function runPreCommitHook() {
  ...
  try {
    execSync(
      `${config.commands.lint} ${config.patterns.typescript} ${config.patterns.javascript}`,
      { stdio: "inherit" },
    );
  ...
  try {
    execSync(
      `prettier --check ${config.patterns.typescript} ${config.patterns.javascript} ${config.patterns.styles}`,
      { stdio: "inherit" },
    );
