...
async function runTests() {
  ...
  try {
    ...
    console.log(
      `✅ Operator verification created. Record ID: ${verification.id}`,
    );

    // 4. Test Edge Function (Requires a listing with is_experience = true)
    console.log("\n--- Test 4: Cashfree Booking Edge Function ---");
    console.log(
      "Note: To test this, you need an active listing marked as is_experience=true.",
    );
    ...
    if (fnError && fnError.message?.includes("Not Found")) {
      console.error(
        '❌ Edge Function "create-booking-order" not found. Need to deploy.',
      );
    } else {
      console.log(
        "✅ Edge Function endpoint reached (response status logged below).",
      );
      console.log(
        `ℹ️ Function Response: ${fnError?.message || "Check detailed logs"}`,
      );
    }
