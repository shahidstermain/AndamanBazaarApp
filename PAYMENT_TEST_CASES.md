...
describe('Payment Verification', () => {
  ...
  it('should reject unauthorized access', async () => {
    ...
    await expect(verifyPayment(mockSupabase, 'order-123'))
...
describe('Webhook Processing', () => {
  ...
  it('should reject invalid webhook signature', async () => {
    ...
    await expect(processWebhook(mockPayload, mockSupabase, 'invalid-signature'))
  ...
  it('should handle stale timestamp', async () => {
    ...
    await expect(processWebhook({}, {}, 'valid-signature', oldTimestamp.toString()))
  ...
  it('should prevent duplicate webhook processing', async () => {
    ...
    const result = await processWebhook(mockPayload, mockSupabase, 'valid-signature', Date.now().toString(), 'webhook-123');
...
describe('Order Creation', () => {
  ...
  it('should create valid boost order', async () => {
    ...
    const mockListing = { id: 'listing-123', user_id: 'user-123', status: 'active' };
    ...
    const result = await createBoostOrder(mockRequest, mockUser, mockListing, mockCashfreeResponse);
  ...
  it('should validate user ownership', async () => {
    ...
    const mockListing = { id: 'listing-123', user_id: 'user-123', status: 'active' }; // Different user

    await expect(createBoostOrder(mockRequest, mockUser, mockListing))
  ...
  it('should validate listing status', async () => {
    const mockRequest = { listing_id: 'listing-123', tier: 'boost' };
    const mockUser = { id: 'user-123', email: 'user@example.com' };
    const mockListing = { id: 'listing-123', user_id: 'user-123', status: 'inactive' };
  ...
  it('should handle Cashfree API failure', async () => {
    ...
    await expect(createBoostOrder(mockRequest, mockUser, mockListing, null, mockCashfreeError))
...
describe('Payment Flow Integration', () => {
  ...
  it('should complete full payment flow successfully', async () => {
    ...
    const orderResponse = await supabase.functions.invoke('create-boost-order', {
      body: {
    ...
    const verificationResponse = await supabase.functions.invoke('verify-payment', {
      body: { order_id },
      headers: {
        Authorization: `Bearer ${testUser.session.access_token}`
      }
    ...
    const finalVerification = await supabase.functions.invoke('verify-payment', {
      body: { order_id },
      headers: {
        Authorization: `Bearer ${testUser.session.access_token}`
      }
...
import { setupTestDatabase, createTestUser, createTestListing } from '../helpers';
...
describe('Payment Idempotency', () => {
  ...
  it('should handle concurrent payment processing', async () => {
    ...
    const locked = results.filter(r => r.status === 'fulfilled' && r.value.data.error?.includes('already being processed'));
...
describe('Webhook Security', () => {
  ...
  it('should prevent replay attacks', async () => {
    ...
    const firstResult = await processWebhook(payload, mockSupabase, validSignature, timestamp, webhookId);
    ...
    const secondResult = await processWebhook(payload, mockSupabaseDuplicate, validSignature, timestamp, webhookId);
  ...
  it('should enforce rate limiting', async () => {
    ...
    await expect(
      processWebhook(payload, mockSupabase, validSignature, timestamp, undefined, clientIP)
  ...
  it('should prevent unauthorized access to payment verification', async () => {
    ...
    await expect(
      verifyPayment(mockSupabase, 'order-123')
    ).rejects.toThrow('Unauthorized');
  });
...
describe('Input Validation', () => {
  ...
  it('should validate required fields', async () => {
    ...
    await expect(createBoostOrder(mockRequest, mockUser))
      .rejects.toThrow('listing_id and tier are required');
  });
  ...
  it('should validate tier options', async () => {
    ...
    await expect(createBoostOrder(mockRequest, mockUser))
      .rejects.toThrow('Invalid tier. Choose: spark, boost, or power');
  });
  ...
  it('should sanitize user input', async () => {
    ...
    await expect(createBoostOrder(maliciousRequest, mockUser))
  ...
  it('should prevent SQL injection', async () => {
    ...
    await expect(createBoostOrder(sqlInjectionRequest, mockUser))
      .rejects.toThrow(/Invalid input/);
  });
});
```
...
test.describe('Payment Journey', () => {
  ...
  test('should complete boost purchase successfully', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="tier-boost.selected"]')).toBeVisible();
...
test.describe('Mobile Payment', () => {
  ...
  test('should work on mobile devices', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="mobile-pay-button"]')).toBeVisible();
...
export async function createTestBoost(supabase: any, listingId: string, userId: string) {
  const { data, error } = await supabase.from('listing_boosts').insert({
...
export function mockWebhookPayload(status: 'PAID' | 'FAILED' | 'EXPIRED') {
  return {
    type: status === 'PAID' ? 'PAYMENT_SUCCESS_WEBHOOK' : 'PAYMENT_FAILED_WEBHOOK',
    data: {
      order: {
