...
export const useActivities = (
  ...
  const processedActivities = useMemo(() => {
    ...
    const filtered = rawActivities.filter((activity: Activity) => {
      if (activity.price < filters.budgetRange[0] || activity.price > filters.budgetRange[1]) return false;
      if (activity.durationMinutes < filters.durationRange[0] || activity.durationMinutes > filters.durationRange[1]) return false;
      if (filters.familyFriendly && !activity.familyFriendly) return false;
      if (filters.requiresSwimming !== undefined && activity.requiresSwimming !== filters.requiresSwimming) return false;
