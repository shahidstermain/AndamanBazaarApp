...
import { validateFileUpload, operatorVerificationSchema } from '../lib/validation';
...
export const OperatorVerificationForm: React.FC<OperatorVerificationFormProps> = ({ profile, onSuccess }) => {
  const { showToast } = useToast();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [address, setAddress] = useState(profile.operator_business_address || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validation = operatorVerificationSchema.safeParse({ business_address: address });
  ...
  if (status === 'verified') {
    return (
      <div className="bg-green-50 border border-green-200 rounded-3xl p-6 flex flex-col items-center text-center space-y-4 animate-in fade-in zoom-in-95">
        ...
        <div>
          <h3 className="text-xl font-black text-green-900">Verified Marketplace Operator</h3>
          <p className="text-green-700 font-medium">You are cleared to list and manage experiences.</p>
        </div>
      </div>
    );
  }
  ...
  if (status === 'pending') {
    return (
      <div className="bg-amber-50 border border-amber-200 rounded-3xl p-6 flex flex-col items-center text-center space-y-4">
        ...
        <div>
          <h3 className="text-xl font-black text-amber-900">Verification Pending</h3>
        ...
        <div className="bg-white/50 backdrop-blur-sm p-4 rounded-2xl w-full text-left border border-amber-100 italic text-sm text-amber-800">
          <p className="font-bold flex items-center gap-2 mb-1"><Building2 size={14} /> Registered Address:</p>
  ...
  return (
    <div className="bg-white rounded-[32px] p-8 shadow-glass border border-warm-100 overflow-hidden relative">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h3 className="text-2xl font-black text-midnight-900">Become an Operator</h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          <label className="text-xs font-black uppercase tracking-widest text-midnight-700 flex items-center gap-2">
            <FileText size={14} className="text-teal-500" /> ID Proof (Aadhar/PAN/License)
          </label>
          <div 
            onClick={() => !isSubmitting && document.getElementById('id-upload')?.click()}
            className={`cursor-pointer border-2 border-dashed rounded-2xl p-6 transition-all flex flex-col items-center gap-3 ${
              idFile ? 'border-teal-500 bg-teal-50/30' : 'border-warm-200 hover:border-teal-300 bg-warm-50'
            }`}
          >
            {idPreview ? (
              <img src={idPreview} alt="ID Preview" className="h-32 rounded-xl border border-teal-200 shadow-sm" />
            ) : idFile ? (
              ...
              <>
                ...
                <div className="text-center">
                  <p className="text-sm font-bold text-midnight-700">Click to upload ID document</p>
                  <p className="text-[10px] text-warm-500 font-bold uppercase mt-1">JPG, PNG or PDF (Max 5MB)</p>
                </div>
              </>
            )}
            <input 
        ...
        {status === 'rejected' && (
          <div className="bg-red-50 border border-red-100 p-4 rounded-2xl flex items-start gap-3">
            ...
            <div className="text-sm">
              <p className="font-black text-red-900 uppercase tracking-wide">Previous Application Rejected</p>
              <p className="text-red-700 font-medium">Please ensure the address matches your ID and the document is clear.</p>
            </div>
          </div>
        )}
        ...
        <p className="text-[10px] text-center text-warm-400 font-bold uppercase tracking-widest px-8">
          By submitting, you agree to AndamanBazaar's Operator terms and 15% marketplace commission structure.
        </p>
      </form>
    </div>
  );
