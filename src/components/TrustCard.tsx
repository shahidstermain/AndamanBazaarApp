...
export const TrustCard: React.FC<TrustCardProps> = ({
  ...
  const joinDate = new Date(seller.created_at).toLocaleDateString("en-IN", {
    month: "short",
    year: "numeric",
  });
  ...
  return (
    <div
      className={`bg-white rounded-2xl border border-warm-200 p-4 space-y-4 shadow-sm ${className}`}
    >
      <div className="flex items-center gap-3">
        <div className="relative">
          <img
            src={
              ...
              `https://ui-avatars.com/api/?name=${encodeURIComponent(seller.name || "User")}&background=0D9488&color=fff`
            }
            alt={seller.name}
        ...
        <div>
          <h3 className="font-bold text-midnight-700 text-sm">
      ...
      <div className="grid grid-cols-2 gap-3 pt-3 border-t border-warm-100">
        <div className="space-y-1">
          ...
          <p
            className={`text-sm font-bold ${responseRate >= 80 ? "text-emerald-600" : responseRate >= 50 ? "text-amber-600" : "text-warm-500"}`}
          >
