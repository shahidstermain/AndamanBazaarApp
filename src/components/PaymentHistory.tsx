...
import { Receipt, Download, ChevronDown, Zap, Rocket, Crown, Clock, CheckCircle, XCircle, Loader2, AlertCircle } from 'lucide-react';
...
const STATUS_BADGES: Record<string, { label: string; className: string; icon: React.ReactNode }> = {
  ...
  paid: { label: 'Active', className: 'bg-green-100 text-green-700', icon: <CheckCircle size={12} /> },
  failed: { label: 'Failed', className: 'bg-red-100 text-red-700', icon: <XCircle size={12} /> },
  refunded: { label: 'Refunded', className: 'bg-blue-100 text-blue-700', icon: <AlertCircle size={12} /> },
  expired: { label: 'Expired', className: 'bg-warm-100 text-warm-600', icon: <Clock size={12} /> },
};
...
export const PaymentHistory: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        ...
        <span className="ml-2 text-warm-500 text-sm">Loading payment history...</span>
      </div>
    );
  }
  ...
  if (boosts.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-bold text-midnight-700 mb-1">No Payments Yet</h3>
        <p className="text-sm text-warm-500">
          When you boost a listing, your payment history and invoices will appear here.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      ...
      {boosts.map(boost => {
        const statusBadge = STATUS_BADGES[boost.status] || STATUS_BADGES.pending;
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                    boost.tier === 'spark' ? 'bg-amber-50' :
                    boost.tier === 'boost' ? 'bg-teal-50' : 'bg-purple-50'
                  }`}>
                    {TIER_ICONS[boost.tier] || <Zap size={16} />}
                  </div>
                  <div>
                    <p className="font-bold text-sm text-midnight-700">{boost.tierName} Boost</p>
                    <p className="text-xs text-warm-500">{boost.durationDays} days · {formatDate(boost.createdAt)}</p>
                  </div>
                </div>

                <div className="text-right">
                  <p className="font-bold text-sm text-midnight-700">{formatInr(boost.amount)}</p>
                  <span className={`inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full ${statusBadge.className}`}>
                    {statusBadge.icon}
            ...
            {isExpanded && invoice && (
              <div className="border-t border-warm-100 bg-warm-50 p-4 animate-fade-in">
                <div className="flex items-center justify-between mb-3">
                  <h4 className="text-xs font-bold text-midnight-700 uppercase tracking-wider">Invoice</h4>
                  <span className="text-[10px] text-warm-500 font-mono">{invoice.invoiceNumber}</span>
                </div>
                ...
                <div className="bg-white rounded-xl border border-warm-100 p-3 space-y-2">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="text-midnight-700 font-medium">{formatDate(invoice.invoiceDate)}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                  ...
                  <div className="border-t border-warm-100 pt-2 mt-2">
                    {invoice.items.map((item, i) => (
                      <div key={i} className="flex justify-between text-xs mb-1">
                        <span className="text-midnight-600 flex-1 pr-2">{item.description}</span>
                        <span className="text-midnight-700 font-bold">{formatInr(item.total)}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="border-t border-warm-200 pt-2 mt-2 flex justify-between">
                    <span className="text-sm font-bold text-midnight-700">Total</span>
                    <span className="text-sm font-black text-midnight-700">{formatInr(invoice.total)}</span>
                  </div>
                </div>
