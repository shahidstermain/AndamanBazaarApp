...
interface ReviewFormProps {
  activityId: string;
  bookingId: string;
  onSubmit: (data: { ratings: ReviewRatings, comment: string, files: File[] }) => Promise<void>;
}

export const ReviewForm: React.FC<ReviewFormProps> = ({ activityId, bookingId, onSubmit }) => {
  ...
  return (
    <div className="bg-white rounded-[24px] border border-warm-200 p-6 shadow-glass">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h3 className="text-xl font-black text-midnight-900">Verified Review</h3>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {(Object.keys(ratings) as Array<keyof ReviewRatings>).map((criteria) => (
            <div key={criteria} className="space-y-2">
              <label className="text-xs font-bold text-warm-500 uppercase tracking-widest flex justify-between">
                <span>{criteria}</span>
                <span className="text-midnight-900">{ratings[criteria]} / 5</span>
              </label>
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((val) => (
                  ...
                  >
                    <Star
                      size={24}
                      className={val <= ratings[criteria] ? "fill-amber-400 text-amber-400" : "fill-warm-100 text-warm-200"}
                    />
                  </button>
                ))}
              </div>
        ...
        <div className="bg-warm-50 p-4 rounded-2xl flex justify-between items-center border border-warm-100">
          <span className="text-sm font-black text-midnight-900 uppercase tracking-widest">Calculated Average Rating</span>
        ...
        <div className="space-y-2">
          <label className="text-xs font-bold text-warm-500 uppercase tracking-widest">Share Your Experience</label>
          <textarea
            value={comment}
        ...
        <div className="space-y-4">
          <label className="text-xs font-bold text-warm-500 uppercase tracking-widest">Add Photos (Max 3)</label>
          <div className="flex gap-4 flex-wrap">
            {files.map((f, i) => (
              <div key={i} className="w-24 h-24 rounded-2xl bg-warm-100 border border-warm-200 overflow-hidden relative group">
                <img src={URL.createObjectURL(f)} alt="review upload" className="w-full h-full object-cover" />
                <button 
                  type="button" 
            ...
            {files.length < 3 && (
              <label className="w-24 h-24 rounded-2xl border-2 border-dashed border-warm-200 flex flex-col items-center justify-center text-warm-400 cursor-pointer hover:bg-warm-50 hover:border-warm-300 transition-colors">
                ...
                <input type="file" accept="image/*" className="hidden" multiple onChange={handleFileDrop} />
              </label>
            )}
          </div>
        </div>
