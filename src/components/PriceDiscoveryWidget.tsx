...
export const PriceDiscoveryWidget: React.FC<PriceDiscoveryWidgetProps> = ({
      ...
      <div className={cn("bg-gray-50 rounded-lg p-4", className)}>
        <div className="flex items-center gap-2 text-gray-500">
          ...
          <span className="text-sm">
            Not enough similar listings to suggest a price. Try checking other
            platforms.
          </span>
        </div>
      </div>
  ...
  const priceDiffPercent =
    currentPriceNum > 0 ? (priceDiff / avgPrice) * 100 : 0;
  ...
  return (
    <div
      className={cn(
        "bg-gradient-to-br from-blue-50 to-teal-50 rounded-lg p-4 border border-blue-100",
        className,
      )}
    >
      {/* Header */}
        ...
        <div className="mb-4">
          ...
          <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${Math.min(Math.abs(priceDiffPercent) + 50, 100)}%`,
              }}
            />
          </div>
          {priceDiffPercent > 20 && (
            <p className="text-xs text-red-600 mt-1">
              Your price is significantly higher than similar listings. Consider
              lowering for faster sale.
            </p>
          )}
          {priceDiffPercent < -20 && (
            <p className="text-xs text-green-600 mt-1">
              Your price is lower than average - great for quick sale! You might
              be undervaluing the item.
            </p>
          )}
        </div>
      )}
