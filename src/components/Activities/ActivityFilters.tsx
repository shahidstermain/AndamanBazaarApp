...
import { Activity, ActivityFilterParams, Island, ActivityType } from '../../types';
import { SlidersHorizontal, MapPin, Tag, Banknote, Clock, Star, Users, Waves, X } from 'lucide-react';
...
const ISLAND_OPTIONS: Island[] = ['Port Blair', 'Havelock', 'Neil Island', 'Baratang', 'Diglipur', 'Long Island'];
const TYPE_OPTIONS: ActivityType[] = ['Scuba Diving', 'Snorkeling', 'Trekking', 'History', 'Leisure', 'Water Sports', 'Beaches'];

export const ActivityFilters: React.FC<ActivityFiltersProps> = ({ filters, setFilters, isOpen, setIsOpen }) => {
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-bold text-teal-600">Up to ₹{filters.budgetRange[1]}</span>
        </div>
        <input
          type="range"
          min="0"
          ...
          title="Budget Range"
          onChange={(e) => setFilters(prev => ({ ...prev, budgetRange: [0, parseInt(e.target.value, 10)] }))}
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-bold text-teal-600">Up to {filters.durationRange[1]} min</span>
        </div>
        <input
          type="range"
          min="0"
          ...
          onChange={(e) => setFilters(prev => ({ ...prev, durationRange: [0, parseInt(e.target.value, 10)] }))}
      ...
      <div className="grid grid-cols-2 gap-4">
        <button
          ...
          onClick={() => setFilters(prev => ({ ...prev, familyFriendly: !prev.familyFriendly }))}
        ...
        >
          <Users size={24} className={filters.familyFriendly ? 'text-blue-500' : 'text-warm-400'} />
          <span className="text-xs font-black uppercase tracking-wider">Family Friendly</span>
        </button>
        ...
        <button
          ...
          onClick={() => setFilters(prev => ({
            ...prev,
            requiresSwimming: prev.requiresSwimming === false ? undefined : false
          }))}
          className={`w-full flex flex-col items-center gap-2 p-4 rounded-2xl border transition-all ${
        ...
        >
          <Waves size={24} className={filters.requiresSwimming === false ? 'text-teal-500' : 'text-warm-400'} />
          <span className="text-xs font-black uppercase tracking-wider">No Swim Req.</span>
        </button>
      </div>
      ...
      <div className="space-y-4">
         <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-bold text-amber-600">{filters.minRating}+ Stars</span>
        </div>
        <input
          type="range"
          min="0"
    ...
    <>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-black text-midnight-900 text-sm">Filter & Sort</span>
        </div>
