...
const createCustomIcon = (score: number, activityType: string) => {
  const colorClass = score >= 90 ? 'bg-orange-500 shadow-orange-500/40' : score >= 70 ? 'bg-amber-400 shadow-amber-400/40' : 'bg-teal-600 shadow-teal-600/40';
...
const ActivityMap: React.FC<ActivityMapProps> = ({ 
  ...
  const memoizedMarkers = useMemo(() => activities.map(activity => (
    <Marker 
      key={activity.id} 
      ...
      <Popup className="activity-popup" maxWidth={300}>
        <div className="p-1 space-y-3 bg-white w-[260px]">
          <div className="aspect-[16/10] rounded-[20px] overflow-hidden relative bg-gradient-to-tr from-teal-600 to-emerald-400 flex items-center justify-center">
            ...
            <div className="text-white font-black text-2xl opacity-50 uppercase tracking-widest">{activity.type}</div>
          </div>
          
          <div className="space-y-1 px-1">
            <h4 className="font-black text-midnight-900 text-base leading-tight line-clamp-2">{activity.title}</h4>
          ...
          <div className="flex items-center justify-between px-1 border-t border-warm-50 pt-3">
            <div className="flex flex-col">
              <span className="text-[10px] text-warm-400 font-bold uppercase tracking-widest leading-none mb-1">Price</span>
              <span className="text-sm font-black text-midnight-900">₹{activity.price}</span>
            </div>
            <div className="flex flex-col items-end">
              <span className="text-[10px] text-warm-400 font-bold uppercase tracking-widest leading-none mb-1">Rating</span>
