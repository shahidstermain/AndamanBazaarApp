...
import { useOfflineSync, createListingSyncHandler, createMessageSyncHandler, createProfileSyncHandler } from '../hooks/useOfflineSync';
...
export const OfflineSyncBanner: React.FC<OfflineSyncBannerProps> = ({ userId, className }) => {
  const handlers = {
  ...
  if (isOnline && pendingCount === 0 && failedCount === 0) {
    ...
    if (lastSyncResult && lastSyncResult.processed > 0 && lastSyncResult.failed === 0) {
      return (
        <div
          className={cn(
        ...
        >
          ...
          <span className="text-sm font-medium">
            {lastSyncResult.succeeded} item{lastSyncResult.succeeded > 1 ? 's' : ''} synced successfully
          </span>
        </div>
      );
    }
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">
            You are offline. {pendingCount > 0 && `${pendingCount} items waiting to sync.`}
          </span>
        </div>
