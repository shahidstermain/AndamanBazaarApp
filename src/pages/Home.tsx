  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-square overflow-hidden bg-warm-100 m-2 rounded-2xl">
        ...
        {/* Trust Badge */}
        {listing.seller &&
          listing.seller.length > 0 &&
          listing.seller[0].trust_level &&
          listing.seller[0].trust_level !== "newbie" && (
            <div className="absolute bottom-2 left-2 flex items-center gap-2">
              <TrustBadge
                level={listing.seller[0].trust_level}
                size="sm"
                showLabel={false}
              />
              <Link
        ...
        {listing.is_featured && !listing.isBoosted && (
          <div
            className={`absolute ${listing.seller && listing.seller.length > 0 && listing.seller[0].trust_level && listing.seller[0].trust_level !== "newbie" ? "top-2 right-2" : "top-2 left-2"} bg-sandy-gradient text-midnight-700 text-3xs font-black uppercase px-2 py-0.5 rounded-full`}
          >
            ✦ Featured
          </div>
        )}
      ...
      <div className="px-3 pb-3 flex-1 flex flex-col justify-between gap-1">
        <div className="space-y-0.5">
          <div className="flex items-center gap-1">
            ...
            {listing.is_official && (
              <span className="text-[8px] font-black text-blue-600 uppercase tracking-tighter shrink-0">
                Team
              </span>
            )}
          </div>
...
const HorizontalListingCard: React.FC<HorizontalCardProps> = ({
  listing,
  rank,
  saved,
  onSave,
  ...
  return (
    <Link
      to={isDemo ? "#" : `/listings/${listing.id}`}
      className="w-44 flex-shrink-0 listing-card group"
      onClick={handleClick}
    >
      <div className="relative aspect-square overflow-hidden bg-warm-100 m-2 rounded-2xl">
        <img
            ...
            <>
              ...
              <div className="flex-1">
                <div className="text-[10px] font-bold">
                  Andaman<span className="text-blue-500">Bazaar</span>
                </div>
                <div className="text-[7px] text-gray-500 tracking-widest uppercase">
                  Local . Trusted
                </div>
              </div>
            </>
      ...
      <div className="px-3 pb-3 flex flex-col gap-1">
        <div className="flex items-center gap-1">
          ...
          {listing.is_official && (
            <span className="text-[7px] font-black text-blue-600 uppercase tracking-tighter shrink-0">
              Team
            </span>
          )}
        </div>
...
export const Home: React.FC = () => {
  ...
  const fetchRecent = async (pageIndex: number) => {
    ...
    try {
      ...
      setRecentListings((prev) =>
        pageIndex === 0 ? all.slice(0, RECENT_PAGE_SIZE) : [...prev, ...paged],
      );
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    navigate(
      searchQuery.trim()
        ? `/listings?q=${encodeURIComponent(searchQuery.trim())}`
        : "/listings",
    );
  };
            ...
            <div className="app-container space-y-8 max-w-3xl mx-auto">
              ...
              <div className="reveal reveal-delay-1">
                <h1 className="font-display text-5xl md:text-6xl xl:text-7xl text-white leading-[1.1] tracking-tight">
                  Buy & Sell <br className="md:hidden" />
        ...
        <section className="px-4 pt-10 mb-10">
          <div className="app-container">
            <div className="section-header reveal">
              <h2 className="font-heading font-extrabold text-xl text-midnight-700 tracking-tight">
                Browse Categories
              </h2>
            ...
            <div className="grid grid-cols-4 md:grid-cols-4 gap-3 md:gap-5">
              {ISLAND_CATEGORIES.map((cat, i) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-14 h-14 md:w-16 md:h-16 rounded-2xl flex items-center justify-center ${cat.bgClass} group-hover:scale-110 transition-transform duration-500 ease-out-expo`}
                    >
                      <Icon size={28} className={cat.textClass} />
                    </div>
                    <span className="text-[11px] md:text-sm font-bold text-midnight-700 text-center leading-tight">
                      {cat.name}
                    </span>
                  </Link>
                );
              ...
              <div className="relative z-10 p-8 md:p-12 flex flex-col md:flex-row md:items-center justify-between gap-8">
                <div className="max-w-xl">
                  ...
                  <p className="text-white/60 text-base md:text-lg font-medium leading-relaxed mb-6">
                    Personalized itineraries, ferry schedules, and
                    island-hopping logic — generated in seconds for your unique
                    travel style.
                  </p>
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex -space-x-3">
                      {[1, 2, 3].map((i) => (
                        <div
                          key={i}
                          className="w-10 h-10 rounded-full border-2 border-midnight-900 bg-warm-200 overflow-hidden shadow-lg"
                        >
                          <img
                    ...
                    <div className="relative z-10 w-full h-full bg-white/5 backdrop-blur-sm border border-white/10 rounded-[40px] p-6 shadow-2xl rotate-3 group-hover:rotate-6 transition-transform duration-700">
                      <div className="space-y-4">
                        {[1, 2, 3].map((i) => (
                          <div
                            key={i}
                            className="h-12 bg-white/5 rounded-xl border border-white/5 animate-pulse"
                            style={{ animationDelay: `${i * 200}ms` }}
                          />
                        ))}
                        <div className="h-20 bg-teal-500/20 rounded-xl border border-teal-500/20 flex items-center justify-center">
                          <Compass
                            className="text-teal-400 animate-spin-slow"
                            size={32}
                          />
                        </div>
                      </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-white/80 text-xs font-heading font-extrabold uppercase tracking-[0.15em]">
                          Flash Deals
                        </span>
                      </div>
                      <p className="text-white font-display text-2xl md:text-3xl leading-tight">
                        Ends in
                      </p>
                    </div>
                      ...
                      {[h, m, ss].map((val, i) => (
                        <React.Fragment key={i}>
                          <div className="bg-midnight-700/80 backdrop-blur-sm text-white text-center px-3 py-2 rounded-xl min-w-[44px]">
                            <div className="text-xl font-heading font-black leading-none">
                              {val}
                            </div>
                            <div className="text-[8px] text-white/40 uppercase font-bold mt-0.5">
                              {["HRS", "MIN", "SEC"][i]}
                            </div>
                          </div>
                          {i < 2 && (
              ...
              <div className="section-header px-0 reveal">
                <div>
                  ...
                  <p className="text-xs text-warm-400 font-medium mt-1">
                    Must go fast! Great local offers
                  </p>
                </div>
                <Link
                  to="/listings?urgent=true"
                  className="section-link text-red-600 font-bold"
                >
                  See All <ArrowRight size={14} />
                </Link>
              </div>
          ...
          <div className="app-container">
            <div className="section-header px-0 reveal">
              <div>
                ...
                <p className="text-xs text-warm-400 font-medium mt-1">
                  Sample listings from across the islands
                </p>
              </div>
              <Link to="/listings?sort=popular" className="section-link">
              ...
              <div className="flex gap-4 w-max pb-2">
                {loadingTrending ? (
                  ...
                  <div className="empty-state py-12 px-8 text-center rounded-3xl border-2 border-dashed border-warm-200 bg-warm-50 min-w-[280px]">
                    <h3 className="font-heading font-bold text-midnight-700 mb-1">
                      🔥 No trending items
                    </h3>
                    <p className="text-sm text-warm-400 mb-4">
                      Post something amazing to get featured here!
                    </p>
                    <Link
                      to="/post"
              ...
              <div className="relative p-6 md:p-8 space-y-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <h3 className="font-heading font-extrabold text-teal-800 text-base md:text-lg">
                    Island Verified Sellers
                  </h3>
                </div>
                <p className="text-teal-700/60 text-sm md:text-base leading-relaxed">
                  GPS-verified locals from across the Andaman Islands. Trade
                  with confidence.
                </p>
                <Link
              ...
              {loadingRecent
                ...
                : (() => {
                    ...
                    const boostedIds = new Set(
                      boostedListings.map((l) => l.id),
                    );
                    const nonBoosted = recentListings.filter(
                      (l) => !boostedIds.has(l.id),
                    );
                    ...
                    return merged.length === 0 ? (
                      <div className="col-span-full empty-state py-16 text-center rounded-3xl border-2 border-dashed border-warm-200 bg-warm-50">
                        <h3 className="font-heading font-bold text-midnight-700 text-lg mb-2">
                          ✨ No fresh arrivals
                        </h3>
                        <p className="text-sm text-warm-400 mb-4">
                          Be the first to post your local goods today!
                        </p>
                        <Link
                          to="/post"
                          className="btn-primary text-sm py-2.5 inline-block"
            ...
            <div className="relative rounded-[32px] overflow-hidden reveal">
              ...
              <div className="relative z-10 p-8 md:p-12">
                ...
                <p className="text-white/50 text-sm md:text-base mb-6 max-w-sm leading-relaxed">
                  Nov–May is peak season. Find the best experiences, stays and
                  local products from verified island sellers.
                </p>
                <Link
