...
export const BecomeOperator: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const addressProofUrl = await uploadToStorage(addressProofFile, 'address_proof');
            ...
            await updateDoc(doc(db, 'profiles', user.uid), { operator_verification_status: 'pending' });
                ...
                <div className="bg-white rounded-[40px] shadow-glass border border-warm-100 overflow-hidden">
                    <div className="bg-midnight-900 p-10 text-white relative">
                        <div className="relative z-10">
                            ...
                            <h1 className="text-4xl font-black mb-2">Operator Verification</h1>
                            <p className="text-teal-400 font-bold uppercase tracking-[0.2em] text-xs">Verify your identity to launch experiences</p>
                        </div>
                    ...
                    <form onSubmit={handleSubmit} className="p-10 space-y-8">
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-midnight-700 flex items-center gap-2">
                                <Building2 size={14} className="text-teal-500" /> Business Name (Optional)
                            </label>
                            <input
                                type="text"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
                            ...
                            <div className="space-y-3">
                                <p className="text-[10px] font-black uppercase tracking-widest text-midnight-700">ID Proof Document</p>
                                <div 
                                    ...
                                    className={`relative border-2 border-dashed rounded-3xl p-6 flex flex-col items-center justify-center min-h-[160px] transition-all cursor-pointer ${
                                        idProofFile ? 'bg-teal-50 border-teal-500' : 'bg-warm-50 border-warm-200 hover:border-teal-300'
                                    }`}
                                >
                                    {idProofFile ? (
                                        <div className="flex flex-col items-center gap-2 text-teal-600 animate-in zoom-in-95">
                                            <CheckCircle2 size={32} />
                                            <span className="text-xs font-bold text-center break-all">{idProofFile.name}</span>
                                        </div>
                                    ) : (
                                        <>
                                            <Upload className="text-warm-400 mb-2" size={28} />
                                            <span className="text-xs font-bold text-midnight-600 text-center">Click to upload ID Proof</span>
                                        </>
                                    )}
                                    <input 
                                        id="id-proof" 
                            ...
                            <div className="space-y-3">
                                <p className="text-[10px] font-black uppercase tracking-widest text-midnight-700">Address Proof Document</p>
                                <div 
                                    onClick={() => document.getElementById('address-proof')?.click()}
                                    className={`relative border-2 border-dashed rounded-3xl p-6 flex flex-col items-center justify-center min-h-[160px] transition-all cursor-pointer ${
                                        addressProofFile ? 'bg-teal-50 border-teal-500' : 'bg-warm-50 border-warm-200 hover:border-teal-300'
                                    }`}
                                >
                                    {addressProofFile ? (
                                        <div className="flex flex-col items-center gap-2 text-teal-600 animate-in zoom-in-95">
                                            <CheckCircle2 size={32} />
                                            <span className="text-xs font-bold text-center break-all">{addressProofFile.name}</span>
                                        </div>
                                    ) : (
                                        <>
                                            <Upload className="text-warm-400 mb-2" size={28} />
                                            <span className="text-xs font-bold text-midnight-600 text-center">Click to upload Address Proof</span>
                                        </>
                                    )}
                                    <input 
                                        id="address-proof" 
                        ...
                        <div className="pt-8">
                            ...
                            <p className="text-center text-[10px] text-warm-400 font-bold uppercase tracking-widest mt-6 max-w-sm mx-auto">
                                By submitting, you agree to become a verified operator according to our marketplace terms.
                            </p>
                        </div>
                    </form>
                </div>
