  ...
  days: [
    {
      ...
      "activities": [
        ...
        },
        {
          ...
          "isBookable": true,
          "operatorId": null,
          "notes": "Carry water and wear light clothing. Tickets available at entrance."
        },
        {
          "id": "act_003",
          "time": "16:00",
      ...
      "activities": [
        {
          ...
          "isBookable": true,
          "operatorId": null,
          "notes": "Book ferry tickets in advance. Carry printed or digital copy."
        }
      ]
    }
  ],
...
export const TripPlanner: React.FC = () => {
  ...
  const getActivityIcon = (type: string) => {
    switch (type.toLowerCase()) {
      case 'transfer': return <Ship size={18} />;
      case 'beaches': return <Waves size={18} />;
      case 'local food': return <Utensils size={18} />;
      case 'photography': return <Camera size={18} />;
      case 'history & culture': return <Compass size={18} />;
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
        ...
        <p className="text-warm-400 font-heading font-bold animate-pulse">Personalizing your itinerary...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-warm-50 pb-24">
      <Seo title="AI Trip Planner | AndamanBazaar" description="Your personalized island adventure itinerary" />
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                <div className="bg-white/10 backdrop-blur-md px-3 py-1 rounded-full border border-white/20 flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-teal-100">AI Generated Plan</span>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-4 text-white/60">
                ...
                <div className="flex items-center gap-1.5">
                  <MapPin size={16} />
                  <span className="text-sm font-bold">Port Blair, Havelock, Neil</span>
                </div>
              </div>
            </div>

            <div className="bg-white/10 backdrop-blur-md border border-white/10 rounded-2xl p-4 min-w-[200px]">
              <p className="text-white/60 text-xs font-bold uppercase tracking-widest mb-1">Estimated Cost</p>
              <div className="flex items-baseline gap-1">
                <span className="text-2xl font-display font-black">₹{itinerary.totalEstimatedCost.toLocaleString()}</span>
                <span className="text-xs text-white/50 font-bold uppercase">INR</span>
              </div>
          ...
          <div className="flex items-center gap-3 overflow-x-auto hide-scrollbar pb-4 -mx-4 px-4 md:mx-0 md:px-0">
            {itinerary.days.map((day) => (
              ...
              >
                ...
                <span className="text-2xl font-display font-black leading-none">{day.dayNumber}</span>
                <span className="text-[8px] font-bold mt-2 truncate w-full text-center px-2">{day.island.split(' (')[0]}</span>
              </button>
            ))}
          </div>
        </div>
          ...
          <div key={day.dayNumber} className="animate-fade-in-up">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-midnight-700">{day.island}</h2>
                  <p className="text-sm text-warm-400 font-medium">{new Date(day.date).toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' })}</p>
                </div>
              </div>
            </div>
                        ...
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className="text-[10px] font-black uppercase tracking-[0.1em] text-warm-400">{activity.type}</span>
                            </div>
                            <h3 className="font-bold text-midnight-700 md:text-lg">{activity.title}</h3>
                          </div>
                          <button 
                          ...
                          <div className="bg-warm-50 rounded-xl p-4 border border-warm-100 mb-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs font-bold text-warm-500">{activity.durationMinutes} Minutes</span>
                              </div>
                              <div className="flex items-center gap-2">
                                <DollarSign size={14} className="text-teal-600" />
                                <span className="text-xs font-bold text-teal-700">₹{activity.estimatedCost.toLocaleString()}</span>
                              </div>
                            </div>
                          </div>
      ...
      <div className="fixed bottom-0 left-0 w-full p-4 pointer-events-none z-50">
        <div className="app-container max-w-lg">
          <div className="bg-midnight-800/95 backdrop-blur-xl border border-white/10 rounded-3xl p-4 shadow-elevated flex items-center justify-between pointer-events-auto">
            <div>
              <p className="text-white/40 text-[10px] font-black uppercase tracking-widest pl-1">Total Trip Value</p>
              <div className="text-xl font-display font-black text-white px-1">₹{itinerary.totalEstimatedCost.toLocaleString()}</div>
            </div>
