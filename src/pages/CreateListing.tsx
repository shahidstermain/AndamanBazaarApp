...
const StepHeader: React.FC<StepHeaderProps> = ({ title, stepLabel }) => (
  <div className="space-y-1">
    ...
    <h2 className="text-2xl font-heading font-black text-midnight-700 leading-tight">
      {title}
    </h2>
  </div>
);
...
const ContinueButton: React.FC<{
  ...
  label?: string;
  loading?: boolean;
}> = ({ onClick, disabled, label = "Continue", loading }) => (
  <button
    onClick={onClick}
...
const BackButton: React.FC<{ onClick: () => void }> = ({ onClick }) => (
  <button
    onClick={onClick}
    className="w-full text-warm-400 font-bold text-sm py-2 hover:text-midnight-700 transition-colors"
  >
    ← Back
  </button>
);
...
export const CreateListing: React.FC = () => {
  ...
  const {
    ...
    removeUpload,
    retryUpload,
    setUploads: setPhotos,
  } = useImageUpload({ maxFiles: 8, maxSizeMB: 10 });
  ...
  useEffect(() => {
    const init = async () => {
      const userId = bypassAuth ? "e2e-user" : await getCurrentUserId();
      if (!userId) {
        navigate("/auth");
        return;
      }
      ...
      if (sourceId) {
        try {
          ...
          if (listing) {
            ...
            setContactPrefs(
              listing.contactPreferences || DEFAULT_CONTACT_PREFERENCES,
            );
            if (listing.category) {
  ...
  const handleRemovePhoto = (id: string) => {
    ...
    if (photo?.id && !photo.file) {
      // If it's an existing server-side image
    ...
    try {
      ...
      if ((data as any).code === "RATE_LIMITED") {
        const retryMinutes = Math.ceil(
          ((data as any).retryAfterSeconds || 3600) / 60,
        );
        showToast(
        ...
        setIsVerified(true);
        showToast(
          (data as any).message || "Island residency verified!",
          "success",
        );
        ...
        const errorMsg =
          data.error ||
          "Location could not be verified as Andaman & Nicobar Islands.";
        showToast(errorMsg, "error");
      }
    } catch (err) {
      console.error("GPS error:", err);
      showToast(
        "Could not access location. Please enable GPS and try again.",
        "error",
      );
    }
    setIsVerifying(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (
        detectPromptInjection(sanitizedTitle) ||
        detectPromptInjection(sanitizedDescription)
      ) {
        showToast(
          "Your listing contains suspicious content. Please revise and try again.",
          "error",
        );
        await logAuditEvent({
      ...
      Object.keys(firestorePayload).forEach(
        (k) => firestorePayload[k] === undefined && delete firestorePayload[k],
      );
      ...
      if (editId && !isRelist) {
        ...
        await logAuditEvent({
          action: isRelist ? "listing_relisted" : "listing_created",
          resource_type: "listing",
          ...
          metadata: {
            category: catId,
            city,
            ...(isRelist ? { relisted_from: relistId } : {}),
          },
        });
      }
      ...
      if (newPhotos.length > 0) {
        const uploads = await uploadListingImages(
          ...
          <div className="bg-white w-full max-w-lg rounded-t-3xl p-8 space-y-6 shadow-2xl">
            ...
            <div className="text-center space-y-1">
              ...
              <p className="text-warm-400 text-sm">
                You have an unsaved listing. Want to pick up where you left off?
              </p>
            </div>
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full py-3 bg-warm-100 text-midnight-700 rounded-2xl font-bold text-sm"
              >
                Start Fresh
              </button>
            </div>
        ...
        <div className="p-8 md:p-12">
          {step === 1 && (
            <div className="space-y-6 animate-fade-in">
              <StepHeader
                title={
                  editId
                    ? "Update Photos"
                    : isRelist
                      ? "Relist — Confirm Photos"
                      : "Add Photos"
                }
                stepLabel={`Step 1 of ${TOTAL_STEPS} — Photos`}
              />
              {isRelist && (
                <div className="flex items-center gap-3 p-4 bg-teal-50 rounded-2xl border border-teal-100">
                  ...
                  <div>
                    <span className="text-teal-700 text-sm font-bold block">
                      Relisting from a previous ad
                    </span>
                    <span className="text-teal-600 text-xs">
                      All details have been carried over. Review and publish!
                    </span>
                  </div>
                </div>
              )}
              <div
                onClick={() =>
                  photos.length < 8 && fileInputRef.current?.click()
                }
              ...
              >
                ...
                {photos.length === 0 ? (
                  <>
                    <Camera
                      size={48}
                      className="text-warm-300 mb-3 group-hover:text-teal-400 transition-colors"
                    />
                    <span className="font-heading font-bold text-midnight-700 text-lg">
                      {COPY.CREATE_LISTING.PHOTO_HINT}
                    </span>
                    <span className="font-medium text-warm-400 text-sm mt-1">
                      Up to 8 · AI-optimized automatically
                    </span>
                  </>
                ) : (
              ...
              <p className="text-center text-warm-400 text-sm font-medium">
                {photos.length} of 8 photos added
              </p>
              {aiLoading && (
                <div className="flex items-center gap-3 p-4 bg-teal-50 rounded-2xl border border-teal-100">
                ...
                <div className="space-y-2">
                  ...
                  {aiSuggestion?.suggested_title &&
                    title !== aiSuggestion.suggested_title && (
                      <button
                        onClick={() => setTitle(aiSuggestion.suggested_title!)}
                        className="flex items-center gap-2 px-4 py-2 bg-teal-50 rounded-full text-teal-700 text-sm font-bold border border-teal-100 hover:bg-teal-100 transition-colors"
                      >
                        <Sparkles size={13} className="text-teal-500" /> Use AI
                        Suggestion
                      </button>
                    )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {aiSuggestion?.estimated_price_range && (
                    <p className="text-xs font-bold text-teal-600 px-1">
                      💡 Market Range: ₹{aiSuggestion.estimated_price_range.low}
                      –₹{aiSuggestion.estimated_price_range.high}
                    </p>
                  )}
                  <div className="flex items-center justify-between px-1 pt-2">
                    <label
                      htmlFor="isNegotiable"
                      className="text-sm font-bold text-midnight-700"
                    >
                      Negotiable
                    </label>
                    <input
                      id="isNegotiable"
                      title="Toggle negotiability"
                      type="checkbox"
                      role="switch"
                  ...
                  {isUrgent && (
                    <p className="text-[10px] font-bold text-amber-600 px-1 mt-1">
                      ⚡ Urgent tag helps you sell faster by attracting quick
                      buyers.
                    </p>
                  )}
                  {isNegotiable && (
                    <div className="space-y-1">
                      ...
                      <input
                        type="number"
                        aria-label="Minimum accepted price"
                        placeholder="Hidden from buyers"
                        value={minPrice}
                        onChange={(e) => setMinPrice(e.target.value)}
                        className="input-island text-sm"
                      />
                    </div>
                  )}
                </div>
              ...
              <ContinueButton
                ...
                disabled={
                  ...
                  !price ||
                  photos.some(
                    (p) => p.status === "compressing" || p.status === "error",
                  )
                }
              />
            </div>
          ...
          {step === 2 && (
            <div className="space-y-6 animate-fade-in">
              <StepHeader
                ...
                stepLabel={`Step 2 of ${TOTAL_STEPS} — Details`}
              />
              <div className="space-y-2">
                <label className="text-xs font-bold text-warm-400 uppercase tracking-widest leading-loose">
                ...
                <div className="flex flex-col gap-3">
                  {CATEGORIES.map((cat) => {
                    ...
                    const labelMap: Record<string, string> = {
                      ...
                      fashion: "Fashion, Clothing & Watch",
                      property: "Property & Real Estate",
                      services: "Services & Jobs",
                      other: "Everything Else",
                    };
                    const subMap: Record<string, string> = {
                      mobiles: "Phones, Accessories",
                      vehicles: "Cars, Bikes, Scooters",
                      home: "Sofas, Beds, Kitchen",
                      fashion: "Men, Women, Kids",
                      property: "Rent, Sale, Land",
                      services: "Cleaning, Plumber, Handyman",
                      other: "Miscellaneous",
                    };
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center justify-between p-4 bg-white rounded-xl border transition-all text-left shadow-sm ${
                          category === cat.name
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div
                            className={`w-12 h-12 rounded-xl flex items-center justify-center ${category === cat.name ? "bg-teal-600 text-white shadow-sm" : "bg-warm-100 text-midnight-600"}`}
                          >
                            <Icon size={24} />
                          </div>
                          <div>
                            <span
                              className={`block font-bold text-sm leading-tight ${category === cat.name ? "text-teal-800" : "text-midnight-700"}`}
                            >
                              {labelMap[cat.id] || cat.name}
                            </span>
                            <span className="block text-xs text-warm-400 mt-1">
                              {subMap[cat.id] || cat.name}
                            </span>
                          </div>
                        </div>
                        <ChevronRight
                ...
                {preCategory &&
                  ["fresh-catch", "produce"].includes(preCategory) && (
                    <p className="text-sm text-teal-600 font-medium mt-2 px-1">
                      {COPY.CREATE_LISTING.CATEGORY_FISH}
                    </p>
                  )}
                {category === "Vehicles" && (
                  <p className="text-sm text-teal-600 font-medium mt-2 px-1">
                    {COPY.CREATE_LISTING.CATEGORY_VEHICLES}
                  </p>
                )}
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                {aiSuggestion?.suggested_description &&
                  description !== aiSuggestion.suggested_description && (
                    <button
                      onClick={() =>
                        setDescription(aiSuggestion.suggested_description!)
                      }
                      className="flex items-center gap-2 px-4 py-2 bg-teal-50 rounded-full text-teal-700 text-sm font-bold border border-teal-100 hover:bg-teal-100 transition-colors"
                    >
                      <Sparkles size={13} className="text-teal-500" /> Use AI
          ...
          {step === 3 && (
            <div className="space-y-6 animate-fade-in">
              <StepHeader
                ...
                stepLabel={`Step 3 of ${TOTAL_STEPS} — Details`}
              />
              ...
              <div className="space-y-4 p-5 bg-warm-50 rounded-3xl border border-warm-200">
                ...
                <div className="space-y-3 pt-1">
                  ...
                  {isNegotiable && (
                    <div className="space-y-2">
                      <label className="text-[10px] font-bold text-warm-400 uppercase tracking-widest">
                        Minimum Accepted Price (Optional)
                      </label>
                      <input
                        type="number"
                        aria-label="Minimum accepted price"
                    ...
                    <button
                      ...
                      className={`p-4 rounded-2xl border-2 text-left transition-all ${
                        condition === opt.value
                          ...
                          : "bg-warm-50 border-warm-200 hover:border-teal-200"
                      }`}
                    >
                      <span
                        className={`font-bold text-sm ${condition === opt.value ? "text-teal-700" : "text-midnight-700"}`}
                      >
                        {opt.label}
                      </span>
                      <p className="text-[10px] text-warm-400 leading-tight mt-0.5">
                        {opt.description}
                      </p>
                    </button>
                  ))}
                ...
                <div className="grid grid-cols-3 gap-2">
                  {ITEM_AGE_OPTIONS.map((opt) => (
                    <button
                      ...
                      className={`py-3 px-1 rounded-xl border-2 text-center transition-all text-[10px] uppercase tracking-tight font-bold ${
                        itemAge === opt.value
                          ? "bg-teal-50 border-teal-400 text-teal-700"
                          : "bg-warm-50 border-warm-200 text-midnight-700"
                      }`}
                    >
                      {opt.label}
                    </button>
                ...
                {accessories.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {accessories.map((acc, i) => (
                      <span
                        key={i}
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-teal-50 text-teal-700 border border-teal-100 rounded-full text-[11px] font-bold"
                      >
                        {acc}
                        <button
                          onClick={() =>
                ...
                <div className="space-y-2">
                  ...
                  <label className="flex items-center gap-4 p-4 bg-warm-50 rounded-2xl border border-warm-200 cursor-pointer hover:border-teal-200 transition-colors">
                    <input
                      ...
                      onChange={(e) =>
                        setContactPrefs((p: ContactPreferences) => ({
                          ...p,
                  ...
                  <label className="flex items-center gap-4 p-4 bg-warm-50 rounded-2xl border border-warm-200 cursor-pointer hover:border-teal-200 transition-colors">
                    <input
                      type="checkbox"
                      checked={contactPrefs.whatsapp || false}
                      onChange={(e) =>
                        setContactPrefs((p: ContactPreferences) => ({
                          ...p,
          ...
          {step === 5 && (
            <div className="text-center py-12 space-y-6 animate-fade-in">
              ...
              <div className="space-y-2">
                <h2 className="text-3xl font-heading font-black text-midnight-700">
                  {editId
                    ? "Listing Updated!"
                    : isRelist
                      ? "Relisted!"
                ...
                <p className="text-warm-400 font-medium">
                  {COPY.SUCCESS.LISTING_PUBLISHED}
                </p>
              ...
              {createdListingId && !editId && (
                <div className="pt-6 border-t border-warm-100 mt-6 animate-fade-in delay-200">
                  <h3 className="font-bold text-midnight-700 text-sm uppercase tracking-widest mb-4">
                    Share your listing
                  </h3>
              ...
              {createdListingId && (
                <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-3xl p-6 border border-amber-200 mt-6 shadow-sm hover:shadow-md transition-shadow text-left">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <h3 className="font-bold text-midnight-700 text-lg mb-1 tracking-tight">
                        Sell it 3x faster
                      </h3>
                      <p className="text-sm text-warm-500 font-medium mb-4">
                        Feature your ad at the top of the{" "}
                        {category || "marketplace"} category and home page.
                      </p>
                      <button
              ...
              <div className="flex flex-col gap-3 pt-6 border-t border-warm-100 mt-6">
                ...
                <button
                  ...
                  className="w-full py-3 text-warm-400 font-bold text-sm hover:text-midnight-700 transition-colors"
                >
                  Go to Profile
                </button>
              </div>
