...
export const Admin: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    setLoading(true);
    try {
      const [totalSnap, activeSnap, pendingSnap, usersSnap, reportsSnap] =
        await Promise.all([
  ...
  if (!isAdmin && loading) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-warm-50">
        ...
        <p className="text-xs font-bold uppercase tracking-widest text-warm-400 mt-4">
          Verifying Access...
        </p>
      </div>
    );
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="inline-flex items-center gap-2 bg-white/10 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest mb-3 border border-white/10">
                <ShieldAlert size={12} className="text-coral-400" /> Admin
                Console
              </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 -mt-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            ...
            <div
              key={i}
              className="bg-white p-6 rounded-3xl shadow-card border border-warm-100 flex flex-col items-center text-center hover:shadow-card-hover transition-all"
            >
              <div
                className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-3 ${stat.bg}`}
              >
                <stat.icon size={24} className={stat.color} />
              </div>
              <span className="text-3xl font-heading font-black text-midnight-700">
                {stat.value}
              </span>
              <span className="text-[10px] font-bold text-warm-400 uppercase tracking-widest mt-1">
                {stat.label}
              </span>
            </div>
          ))}
        ...
        <div className="bg-white rounded-[32px] shadow-card border border-warm-200 overflow-hidden">
          <div className="p-6 md:p-8 border-b border-warm-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h2 className="text-xl font-heading font-black text-midnight-700">
                Moderation Queue
              </h2>
              <p className="text-sm text-warm-400 font-medium">
                {stats.pendingReports > 0
                  ? COPY.ADMIN.PENDING_QUEUE(stats.pendingReports)
                  : "Review reported listings and user flags"}
              </p>
            </div>
              ...
              <tbody className="divide-y divide-warm-100">
                {filteredReports.length === 0 ? (
                  <tr>
                    <td
                      colSpan={6}
                      className="px-6 py-12 text-center text-warm-400 font-medium"
                    >
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="font-bold text-midnight-700 text-sm">
                      ...
                      <td className="px-6 py-4">
                        ...
                        {report.description && (
                          <p
                            className="text-xs text-warm-500 mt-1 max-w-xs truncate"
