  ...
  const fetchListings = useCallback(
    async (reset = true) => {
      ...
      try {
        ...
        if (showVerifiedOnly)
          constraints.push(where("isLocationVerified", "==", true));
        ...
        if (filters.minPrice !== "" && !isNaN(Number(filters.minPrice))) {
          results = results.filter(
            (l) => (l.price ?? 0) >= Number(filters.minPrice),
          );
        }
        if (filters.maxPrice !== "" && !isNaN(Number(filters.maxPrice))) {
          results = results.filter(
            (l) => (l.price ?? 0) <= Number(filters.maxPrice),
          );
        }
        if (filters.durations.length > 0) {
          results = results.filter((l) => {
            ...
            if (filters.durations.includes("< 2 hours") && mins < 120)
              return true;
            if (
              filters.durations.includes("Half Day") &&
              mins >= 120 &&
              mins <= 360
            )
              return true;
            if (filters.durations.includes("Full Day") && mins > 360)
              return true;
            return false;
          });
        }
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-warm-200 shadow-sm">
              <form onSubmit={handleSearchSubmit} className="relative flex-1">
                ...
                {searchQuery && (
                  <button
                    ...
                    type="button"
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-warm-300 hover:text-midnight-700"
                  >
                    <X size={16} />
                  </button>
                )}
              ...
              <div className="flex items-center gap-4 px-2 overflow-x-auto no-scrollbar">
                ...
                >
                  <Sparkles
                    size={12}
                    className={showVerifiedOnly ? "animate-pulse" : ""}
                  />
                  Verified
                </button>
                ...
                >
                  <Bell
                    size={12}
                    className={showUrgentOnly ? "animate-bounce-subtle" : ""}
                  />
                  Urgent
                </button>
              </div>
              ...
              {loading ? (
                ...
                <div className="col-span-full py-24 text-center space-y-5 bg-red-50 rounded-3xl border-2 border-dashed border-red-200 animate-fade-in">
                  ...
                  <div className="space-y-1.5">
                    <h3 className="text-xl font-heading font-bold text-red-700">
                      Connection Trouble
                    </h3>
                    <p className="text-red-600/80 text-sm max-w-xs mx-auto">
                      {error}
                    </p>
                  </div>
                  <button
                ...
                <div className="col-span-full py-24 text-center space-y-5 bg-warm-50 rounded-3xl border-2 border-dashed border-warm-200 animate-fade-in">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 justify-center mt-4">
                    {hasActiveFilters ? (
                      ...
                      <Link to="/post" className="btn-primary text-sm py-2.5">
                        Post a Listing
                      </Link>
                    )}
                  </div>
              ...
              >
                {loadingMore && (
                  <div className="flex items-center gap-3">
                    <Loader2
                      size={20}
                      className="animate-spin text-ocean-600"
                    />
...
}> = ({ listing, isFavorited, onToggleFavorite }) => {
  const imageUrl =
    ...
    >
      <div className="relative aspect-square bg-warm-100 m-2 rounded-2xl overflow-hidden">
        ...
        <button
          ...
          className={`absolute top-2 right-2 w-9 h-9 rounded-full flex items-center justify-center shadow-glass z-10 transition-all active:scale-90 ${
            isFavorited ? "bg-coral-500" : "bg-white/90 backdrop-blur-sm"
          }`}
          aria-label={isFavorited ? "Unsave" : "Save"}
        >
          <Heart
            size={15}
            className={isFavorited ? "text-white fill-white" : "text-warm-400"}
            strokeWidth={2}
          />
        </button>
        {listing.is_featured && (
      ...
      <div className="px-3 pb-3 flex flex-col flex-1">
        <span className="font-heading font-black text-teal-600 text-base">
          ₹{listing.price?.toLocaleString("en-IN")}
        </span>
        <h3 className="text-[12px] font-bold text-midnight-700 line-clamp-2 leading-tight mt-0.5 group-hover:text-teal-600 transition-colors">
          {listing.title}
        </h3>
      </div>
    </Link>
