...
export const listingSchema = z
  .object({
    ...
    item_age: z
      .enum(["<1m", "1-6m", "6-12m", "1-2y", "2-5y", "5y+"])
      .optional()
      .nullable(),
    ...
    accessories: z
      .array(
        z
          .string()
          .min(1, "Accessory name cannot be empty")
          .max(50, "Accessory name too long"),
      )
      .max(15, "Maximum 15 accessories")
      .default([]),
  ...
  .refine(
    ...
    {
      message: "Minimum price must be less than the listing price",
      path: ["min_price"],
    },
  );
...
export const messageSchema = z.object({
  message_text: z
    ...
    .refine((val) => !/<script/i.test(val), "Message contains invalid content"),
...
export type OperatorVerificationInput = z.infer<
  typeof operatorVerificationSchema
>;
...
export const validateFileUpload = (
  ...
  // Check file name for suspicious patterns, including double extensions (e.g. shell.php.jpg)
  const normalizedName = file.name.toLowerCase().trim();
  if (
    /\.(?:exe|sh|bat|cmd|ps1|php\d*|phtml|asp|aspx|jsp|js)(?:\.|$)/i.test(
      normalizedName,
    )
  ) {
