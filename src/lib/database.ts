...
import { collection, doc, getDoc, getDocs, addDoc, updateDoc, deleteDoc, query, where, orderBy, limit, startAfter, onSnapshot, serverTimestamp, Timestamp } from 'firebase/firestore';
...
export const getListings = async (options: {
  ...
  try {
    ...
    if (options.startAfter) constraints.push(startAfter(doc(firestoreDb, 'listings', options.startAfter)));

    const querySnapshot = await getDocs(query(collection(firestoreDb, 'listings'), ...constraints));
    const listings = querySnapshot.docs.map(d => ({ id: d.id, ...d.data() })) as Listing[];
    return { listings, hasMore: listings.length === (options.limit || 10), lastDoc: querySnapshot.docs[querySnapshot.docs.length - 1] };
  } catch (error) {
...
export const createListing = async (listingData: Omit<Listing, 'id' | 'createdAt' | 'updatedAt'>): Promise<Listing> => {
  try {
    ...
    const docRef = await addDoc(collection(firestoreDb, 'listings'), { ...listingData, createdAt: now, updatedAt: now });
    const localNow = Timestamp.now();
    return { id: docRef.id, ...listingData, createdAt: localNow, updatedAt: localNow } as Listing;
  } catch (error) {
...
export const updateListing = async (id: string, updates: Partial<Listing>): Promise<Listing> => {
  try {
    await updateDoc(doc(firestoreDb, 'listings', id), { ...updates, updatedAt: serverTimestamp() });
...
export const getUserChats = async (userId: string): Promise<Chat[]> => {
  try {
    const [buyerSnap, sellerSnap] = await Promise.all([
      getDocs(query(collection(firestoreDb, 'chats'), where('isActive', '==', true), where('buyerId', '==', userId), orderBy('lastMessageAt', 'desc'))),
      getDocs(query(collection(firestoreDb, 'chats'), where('isActive', '==', true), where('sellerId', '==', userId), orderBy('lastMessageAt', 'desc')))
    ]);
...
export const subscribeToListing = (id: string, callback: (listing: Listing | null) => void): (() => void) => {
...
export const subscribeToUserChats = (userId: string, callback: (chats: Chat[]) => void): (() => void) => {
  const buyerUnsub = onSnapshot(
    query(collection(firestoreDb, 'chats'), where('isActive', '==', true), where('buyerId', '==', userId)),
    (snap) => callback(snap.docs.map(d => ({ id: d.id, ...d.data() } as Chat)))
  );
  const sellerUnsub = onSnapshot(
    query(collection(firestoreDb, 'chats'), where('isActive', '==', true), where('sellerId', '==', userId)),
    (snap) => callback(snap.docs.map(d => ({ id: d.id, ...d.data() } as Chat)))
  );
  return () => { buyerUnsub(); sellerUnsub(); };
};
