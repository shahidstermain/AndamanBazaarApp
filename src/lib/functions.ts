...
export const verifyLocation = async (
  ...
  try {
    ...
    if (!fnUrl) {
      logger.error(
        "Firebase location verification function URL not configured",
      );
    ...
    if (!response.ok) {
      logger.error("Location verification error", {
...
export const batchModerateContent = async (
  requests: AiModerationRequest[],
): Promise<AiModerationResponse[]> => {
  const promises = requests.map((request) => moderateContent(request));
  return await Promise.all(promises);
};

export const batchCreateInvoices = async (
...
export const checkFunctionHealth = async (): Promise<{
  provider: string;
  healthy: boolean;
  latency?: number;
}> => {
