...
const IS_DEV = import.meta.env.DEV || import.meta.env.VITE_ENV === 'development';
...
export const logger = {
  ...
  error(message: string, errorOrMeta?: unknown, meta?: LogMeta): void {
    ...
    if (errorOrMeta instanceof Error) {
      console.error(format('error', message, { ...meta, error: errorOrMeta.message, stack: errorOrMeta.stack }));
    } else {
