...
export const server = setupServer(
  ...
  rest.post('https://identitytoolkit.googleapis.com/v1/accounts:signUp', (req, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json({
        idToken: 'mock-id-token',
  ...
  // Firestore endpoints
  rest.get('https://firestore.googleapis.com/v1/projects/test-project/databases/(default)/documents/listings', (req, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json({
        documents: [
  ...
  rest.post('https://firestore.googleapis.com/v1/projects/test-project/databases/(default)/documents/listings', (req, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json({
  ...
  // Storage endpoints
  rest.post('https://storage.googleapis.com/upload/storage/v1/b/test-project.appspot.com/o', (req, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json({
  ...
  rest.post('https://api.cashfree.com/pg/orders/:orderId/verify', (req, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json({
...
export const mockStorage = {
  ref: vi.fn(() => ({
    ...
    getDownloadURL: vi.fn().mockResolvedValue('https://example.com/test-image.jpg'),
    delete: vi.fn().mockResolvedValue(undefined)
  }))
}
...
export const IntegrationTestHelpers = {
  ...
  mockNetworkCondition: (condition: 'offline' | 'slow' | 'fast') => {
    const conditions = {
      ...
      slow: { online: true, latency: 2000, downloadThroughput: 1000, uploadThroughput: 500 },
      fast: { online: true, latency: 50, downloadThroughput: 10000, uploadThroughput: 5000 }
    }
...
export const IntegrationTestExamples = {
  ...
  testAuthFlow: async () => {
    const { renderWithIntegrationProviders, screen, fireEvent, userEvent } = await import('@testing-library/react')
  ...
  testListingCreation: async () => {
    const { renderWithIntegrationProviders, screen, userEvent } = await import('@testing-library/react')
