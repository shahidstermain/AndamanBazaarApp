...
import {
  ...
  parseWebhookEvent,
} from "../functions/src/utils/webhookUtils";
...
describe("Webhook Signature Validation", () => {
  ...
  describe("verifyWebhookSignature", () => {
    ...
    it("should be case-sensitive", () => {
      ...
      const incorrectCaseSignature =
        correctSignature.toLowerCase() === correctSignature
      ...
      const finalIncorrectSignature =
        incorrectCaseSignature === correctSignature
          ? correctSignature + "a"
          : incorrectCaseSignature;

      const result = verifyWebhookSignature(
        testPayload,
        finalIncorrectSignature,
        testSecretKey,
      );
      expect(result).toBe(false);
    });
    ...
    it("should handle different payload formats", () => {
      ...
      expect(
        verifyWebhookSignature(jsonPayload, jsonSignature, testSecretKey),
      ).toBe(true);
      expect(
        verifyWebhookSignature(stringPayload, stringSignature, testSecretKey),
      ).toBe(true);
      expect(
        verifyWebhookSignature(jsonPayload, stringSignature, testSecretKey),
      ).toBe(false);
    });
  });
