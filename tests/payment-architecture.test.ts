...
describe('Payment Architecture End-to-End Tests', () => {
    ...
    it('should create a boost order with valid parameters', async () => {
      ...
      const mockCashfreeResponse = {
        data: {
          ...
          payment_link: 'https://sandbox.cashfree.com/pg/view/order/session_123'
        }
      };
      ...
      expect(Cashfree.PGCreateOrder).toHaveBeenCalledWith('2023-08-01', expect.objectContaining(expectedPayload));
    });
  ...
  describe('3. Webhook Processing', () => {
    it('should verify webhook signature correctly', async () => {
      ...
      expect(() => {
        Cashfree.PGVerifyWebhookSignature(mockSignature, mockRawBody, mockTimestamp);
      }).not.toThrow();
    });
  ...
  describe('4. Listing Boost Activation', () => {
    it('should update listing with featured status', async () => {
      ...
      expect(updatedListing.featured_until).toBe(boostActivation.featured_until);
    });
