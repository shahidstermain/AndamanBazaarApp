...
describe('sanitizeErrorMessage()', () => {
  it('redacts email addresses', () => {
    const result = sanitizeErrorMessage({ message: 'Error for user@example.com' })
  ...
  it('returns generic message for database errors', () => {
    const result = sanitizeErrorMessage({ message: 'database connection failed' })
    expect(result).toBe('An error occurred. Please try again later.')
  })

  it('returns generic message for SQL errors', () => {
    const result = sanitizeErrorMessage({ message: 'SQL syntax error at position 12' })
    expect(result).toBe('An error occurred. Please try again later.')
  })
...
describe('isTransientError()', () => {
  ...
  it('identifies "failed to fetch" as transient', () => {
    expect(isTransientError({ message: 'failed to fetch', name: 'TypeError' })).toBe(true)
  })
...
describe('retryAsync()', () => {
  ...
  it('retries on transient error and succeeds', async () => {
    ...
    const result = await retryAsync(op, { maxAttempts: 3, baseDelayMs: 1, jitterMs: 0 })
