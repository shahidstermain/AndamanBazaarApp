...
import { supabase, isSupabaseConfigured } from '../../src/lib/__mocks__/supabase'
...
describe('AuthView extended flows', () => {
  beforeEach(() => {
    ...
    vi.mocked(supabase.auth.signInWithPassword).mockResolvedValue({ data: { session: null }, error: null } as any)
  ...
  it('shows localized wrong-password message for invalid login credentials', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Secret Password/i), { target: { value: 'wrong-pass' } })
  ...
  it('shows email-not-confirmed guidance with resend action', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Email Address/i), { target: { value: 'verifyme@example.com' } })
    ...
    await waitFor(() => {
      ...
      expect(screen.getByRole('button', { name: /Resend to Current Origin/i })).toBeTruthy()
    })
  })
  ...
  it('blocks signup when password is weak and does not call signUp', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Secret Password/i), { target: { value: 'weak' } })
    fireEvent.click(screen.getByRole('button', { name: /Create Island Account/i }))
