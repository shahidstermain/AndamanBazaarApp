...
const baseValidListing = {
  ...
  description: 'Well maintained engine in excellent condition. Suitable for island fishing use.',
  price: 12000,
...
describe('Create listing validation (deterministic)', () => {
  ...
  it('rejects price higher than ₹10,000,000', () => {
    const result = listingSchema.safeParse({ ...baseValidListing, price: 10000001 })
    expect(result.success).toBe(false)
  })

  it('rejects title with disallowed characters', () => {
    const result = listingSchema.safeParse({ ...baseValidListing, title: 'Title<script>alert(1)</script>' })
    expect(result.success).toBe(false)
  })
})
