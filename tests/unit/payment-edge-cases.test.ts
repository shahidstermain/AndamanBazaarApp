...
const resolveBoostStatusAfterWebhook = (status: 'pending' | 'paid' | 'failed', webhookDelivered: boolean) => {
  if (webhookDelivered && status === 'pending') return 'paid'
  return status
}

const applyConcurrentActivation = (statuses: Array<'pending' | 'paid' | 'failed'>) => {
  let activated = false
...
describe('Payment edge-case behavior (CI-stable)', () => {
  ...
  it('handles transient network errors and recovers with retryAsync', async () => {
    ...
    const result = await retryAsync(op, { maxAttempts: 4, baseDelayMs: 1, jitterMs: 0 })
