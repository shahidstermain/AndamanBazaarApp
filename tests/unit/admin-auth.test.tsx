  ...
  it("shows dashboard when user has admin role", async () => {
    ...
    vi.mocked(getDocs).mockImplementation(async (q: any) => {
      ...
      if (path.includes("user_roles"))
        return createMockSnap(1, { role: "admin", user_id: "admin-user" });
      if (path.includes("listings"))
        return createMockSnap(q?.type === "query" ? 51 : 50);
      if (path.includes("reports"))
        return createMockSnap(5, {
          reason: "test_reason",
          status: "pending",
    ...
    await waitFor(
      () => {
        ...
        expect(
          screen.getByText(/5 listings queue mein hain/),
        ).toBeInTheDocument();
