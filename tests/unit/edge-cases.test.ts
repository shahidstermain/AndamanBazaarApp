...
describe("Maximum Field Length Boundaries", () => {
  const baseValid = {
    ...
    description:
      "A valid description that is definitely more than twenty characters long for testing.",
    price: 500,
    category_id: "other",
...
describe("Search Query — Injection Attempts", () => {
  it("rejects single quote SQL injection", () => {
    expect(searchQuerySchema.safeParse({ query: "' OR 1=1 --" }).success).toBe(
      false,
    );
  });

  it("rejects double quote injection", () => {
    expect(searchQuerySchema.safeParse({ query: '" OR ""="' }).success).toBe(
      false,
    );
  });
  ...
  it("rejects semicolon injection", () => {
    expect(
      searchQuerySchema.safeParse({ query: "; DROP TABLE listings" }).success,
    ).toBe(false);
  });

  it("rejects backslash injection", () => {
    expect(searchQuerySchema.safeParse({ query: "test\\; --" }).success).toBe(
      false,
    );
  });
  ...
  it("allows normal search with spaces and numbers", () => {
    expect(
      searchQuerySchema.safeParse({ query: "iphone 15 pro max" }).success,
    ).toBe(true);
  });

  it("allows search with Hindi characters", () => {
    // Note: current schema doesn't block Unicode, only ', ", ;, \
    expect(searchQuerySchema.safeParse({ query: "ताजा मछली" }).success).toBe(
      true,
    );
  });
});
...
describe("Rate Limiting — Concurrent Burst", () => {
  it("blocks rapid burst exceeding limit", () => {
    ...
    for (let i = 0; i < 25; i++) {
      results.push(
        checkRateLimit(key, { maxRequests: 10, windowSeconds: 60 }).allowed,
      );
    }
