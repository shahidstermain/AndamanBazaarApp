...
describe('Boost activation logic (deterministic)', () => {
  ...
  it('computes featured_until based on duration days with frozen time', () => {
    ...
    expect(computeFeaturedUntil(now, spark.durationDays).toISOString()).toBe('2026-03-04T00:00:00.000Z')
    expect(computeFeaturedUntil(now, boost.durationDays).toISOString()).toBe('2026-03-08T00:00:00.000Z')
    expect(computeFeaturedUntil(now, power.durationDays).toISOString()).toBe('2026-03-31T00:00:00.000Z')
  })
