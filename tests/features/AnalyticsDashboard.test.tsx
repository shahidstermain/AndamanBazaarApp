...
describe('ListingPerformanceCard', () => {
  ...
  it('shows status badge with green for active', () => {
    render(<ListingPerformanceCard analytics={makeAnalytics({ status: 'active' })} />);
  ...
  it('shows status badge with blue for sold', () => {
    render(<ListingPerformanceCard analytics={makeAnalytics({ status: 'sold' })} />);
  ...
  it('shows "No activity" when last_activity_at is null', () => {
    render(
      <ListingPerformanceCard analytics={makeAnalytics({ last_activity_at: null })} />
    );
  ...
  it('renders comparison section when provided', () => {
    render(
      <ListingPerformanceCard
        ...
        comparison={[
          { metric: 'views', listing_value: 150, category_avg: 80, percentile: 75 },
        ]}
      />
    );
