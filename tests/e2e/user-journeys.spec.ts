...
class CreateListingPage {
  ...
  async expectSuccessScreen() {
    await expect(this.page.locator('text=Published!')).toBeVisible({ timeout: 10000 })
  }
}
...
class HomePage {
  ...
  async expectListingsVisible() {
    await expect(this.page.locator('.listing-card').first()).toBeVisible({ timeout: 5000 })
  }
}
...
class AdminPage {
  ...
  async expectDashboard() {
    await expect(this.page.locator('text=Dashboard Overview')).toBeVisible({ timeout: 10000 })
  }
...
test.describe('Journey 1: Signup → Create Listing', () => {
  ...
  test('signup form validates password strength', async ({ page }) => {
    ...
    await expect(page.locator('text=Password too weak')).toBeVisible({ timeout: 3000 })
  })
})
...
test.describe('Journey 2: Login → Search → Browse', () => {
  ...
  test('listings page shows category filters', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("Fresh Catch")')).toBeVisible()
  })
})
...
test.describe('Journey 5: Empty State Handling', () => {
  test('listings page shows empty state with local humor copy', async ({ page }) => {
