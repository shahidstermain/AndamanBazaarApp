...
test.describe("User journeys extended (non-blocking)", () => {
  ...
  test("boost payment journey can reach payment initiation state", async ({
    page,
  }) => {
    ...
    const boostBtn = page
      .locator('button:has-text("Boost"), button:has-text("Promote")')
      .first();
  ...
  test("session expiry mid-action shows auth path recovery", async ({
    page,
  }) => {
