...
test.describe('E2E Performance Tests', () => {
  ...
  test('should load images progressively without layout shifts', async ({ page }) => {
    await page.goto('/')
  ...
  test('should handle real-time updates efficiently', async ({ page }) => {
    ...
    const memoryMetrics = await page.evaluate(async () => {
      ...
      for (let i = 0; i < messageCount; i++) {
        ...
        // Track peak memory
        if (memory) {
          metrics.peakMemory = Math.max(metrics.peakMemory, memory.usedJSHeapSize)
        }
  ...
  test('should optimize bundle loading with code splitting', async ({ page }) => {
    await page.goto('/')
        ...
        for (const entry of list.getEntries()) {
          if (entry.entryType === 'resource') {
            ...
            if (resource.name.includes('.js') || resource.name.includes('.css')) {
              ...
              metrics.chunkCount++
              metrics.loadTimes.push(resource.responseEnd - resource.requestStart)
    ...
    const averageLoadTime = networkMetrics.loadTimes.reduce((a, b) => a + b, 0) / networkMetrics.loadTimes.length
