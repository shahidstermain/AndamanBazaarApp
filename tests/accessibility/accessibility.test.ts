...
test.describe("Screen Reader Tests", () => {
  ...
  test("should have proper error messages", async ({ page }) => {
    ...
    const errorMessages = await page
      .locator('[role="alert"], .error, .text-red-600')
      .all();
    ...
    for (const error of errorMessages) {
      const isAssociated = await error.evaluate((el) => {
        ...
        const describedBy = document.querySelectorAll(
          `[aria-describedby="${id}"]`,
        );
  ...
  test("should have proper loading states", async ({ page }) => {
    ...
    const loadingElements = await page
      .locator('[aria-busy="true"], .loading, .animate-pulse')
      .all();
...
test.describe("WCAG 2.1 Level AA Compliance", () => {
  test("should meet color contrast requirements", async ({ page }) => {
    ...
    const largeText = await page
      .locator("h1, h2, h3, .text-lg, .text-xl, .text-2xl")
      .all();
    ...
    const normalText = await page
      .locator("p, span, a:not(.text-lg), button:not(.text-lg)")
      .all();
  ...
  test("should provide text alternatives for non-text content", async ({
    ...
    const icons = await page
      .locator('[class*="icon"], .material-symbols-outlined')
      .all();
