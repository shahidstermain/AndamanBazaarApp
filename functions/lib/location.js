...
exports.cleanupLocationVerifications =
  exports.sendLocationBasedNotifications =
...
const locationRuntime = functions.runWith({
  secrets: secrets_1.ADMIN_SECRET_BINDINGS,
});
...
exports.getLocationHistory = locationRuntime.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
    ...
    const { limit = 10 } = data;
    try {
      const snapshot = await admin_1.admin
        .firestore()
        .collection("location_verifications")
...
// Get Nearby Listings
exports.getNearbyListings = locationRuntime.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
    ...
    try {
      ...
      const listings = await admin_1.admin
        .firestore()
        .collection("listings")
        .where("status", "==", "active")
        .where("isActive", "==", true)
...
exports.cleanupLocationVerifications = locationRuntime.pubsub
  ...
  .onRun(async (context) => {
    ...
    const oldVerifications = await admin_1.admin
      .firestore()
      .collection("location_verifications")
