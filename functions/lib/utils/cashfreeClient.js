...
exports.initializeCashfree =
  exports.Cashfree =
  exports.createRefund =
  exports.getCashfreeOrderStatus =
...
Object.defineProperty(exports, "Cashfree", {
  enumerable: true,
  get: function () {
    return cashfree_pg_1.Cashfree;
  },
});
...
Object.defineProperty(exports, "verifyWebhookSignature", {
  enumerable: true,
  get: function () {
    return webhookUtils_1.verifyWebhookSignature;
  },
});
Object.defineProperty(exports, "parseWebhookEvent", {
  enumerable: true,
  get: function () {
    return webhookUtils_1.parseWebhookEvent;
  },
});
...
const createCashfreeOrder = async (request) => {
  try {
    ...
    v2_1.logger.info(`Creating Cashfree order: ${request.orderId}`, {
      orderRequest,
    });
    ...
    v2_1.logger.error(
      `Failed to create Cashfree order: ${request.orderId}`,
      error,
    );
    throw new Error(
...
const getCashfreeOrderStatus = async (orderId) => {
  try {
    ...
    const responseData = response.data;
    const paymentsResponse = await cashfree_pg_1.Cashfree.PGOrderFetchPayments(
      "2025-01-01",
      orderId,
    );
    ...
    v2_1.logger.error(
      `Failed to fetch Cashfree order status: ${orderId}`,
      error,
    );
    throw new Error(
...
const createRefund = async (request) => {
  try {
    ...
    v2_1.logger.info(`Creating refund for order: ${request.orderId}`, {
      refundRequest,
    });
    ...
    const responseData = response.data;
    v2_1.logger.info(
      `Refund created successfully: ${responseData.cf_refund_id}`,
      {
