...
exports.sendAdminNotification = exports.onNewFavorite = exports.onListingSold = exports.onBoostActivated = exports.onNewChatMessage = void 0;
...
const messaging = admin_1.admin.messaging();
// ── Helpers ──────────────────────────────────────────────────
...
async function sendPush(userId, title, body, data = {}) {
    ...
    try {
        ...
        v2_1.logger.info('Push sent', { userId, title });
    }
    catch (err) {
        if (err.code === 'messaging/registration-token-not-registered' ||
            err.code === 'messaging/invalid-registration-token') {
            await db.collection('fcmTokens').doc(userId).update({ token: null });
            v2_1.logger.info('Stale FCM token cleared', { userId });
        }
        else {
            v2_1.logger.error('Push send failed', { userId, err: err.message });
        }
    }
}
...
exports.onNewChatMessage = functions.firestore
    ...
    if (!recipientId)
        return;
    const listingTitle = chat.listingTitle || 'your listing';
    await sendPush(recipientId, 'New message', msg.text
...
exports.onBoostActivated = functions.firestore
    ...
    if (before.status !== 'paid' && after.status === 'paid') {
        ...
        const expiresStr = expiresAt
            ...
            : '';
        await sendPush(after.userId, '🚀 Boost Activated!', `"${listingTitle}" is now boosted${expiresStr ? ` until ${expiresStr}` : ''}.`, {
...
exports.onListingSold = functions.firestore
    ...
    if (before.status !== 'sold' && after.status === 'sold') {
        ...
        await sendPush(after.userId, '🎉 Listing Sold!', `Congratulations! "${after.title || 'Your listing'}" has been marked as sold.`, {
            type: 'listing_sold',
            listingId,
...
exports.onNewFavorite = functions.firestore
    ...
    if (!listingId || !favoritedBy)
        return;
    const listingSnap = await db.collection('listings').doc(listingId).get();
    if (!listingSnap.exists)
        return;
    const listing = listingSnap.data();
    if (!listing.userId || listing.userId === favoritedBy)
        return;
    await sendPush(listing.userId, '❤️ Someone liked your listing', `"${listing.title || 'Your listing'}" was added to favourites.`, {
        type: 'new_favorite',
        listingId,
        url: `/listing/${listingId}`,
    });
...
exports.sendAdminNotification = functions.https.onCall(async (data, context) => {
    ...
    v2_1.logger.info('Admin broadcast sent', { sent, total: tokensSnap.size });
    return { sent };
});
