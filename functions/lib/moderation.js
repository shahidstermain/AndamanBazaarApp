...
exports.getModerationStats = exports.autoModerateListing = exports.getModerationHistory = exports.batchModerateContent = exports.moderateContent = void 0;
const functions = __importStar(require("firebase-functions"));
const generative_ai_1 = require("@google/generative-ai");
const admin_1 = require("./utils/admin");
const secrets_1 = require("./utils/secrets");
const moderationRuntime = functions.runWith({ secrets: secrets_1.MODERATION_SECRET_BINDINGS });
...
const moderateContentInternal = async (data, callerUid) => {
    ...
    await admin_1.admin.firestore().collection('content_moderations').add({
        userId,
        contentType,
...
// Moderate Content
exports.moderateContent = moderationRuntime.https.onCall(async (data, context) => {
    if (!context.auth) {
...
exports.batchModerateContent = moderationRuntime.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const { requests } = data;
...
exports.getModerationHistory = moderationRuntime.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
      ...
      `;
        ...
        if (moderationResult.approved) {
            await snap.ref.update({
                ...
                moderationApprovedAt: admin_1.admin.firestore.FieldValue.serverTimestamp(),
            });
        }
        else {
            await snap.ref.update({
                ...
                moderationFlaggedAt: admin_1.admin.firestore.FieldValue.serverTimestamp(),
            });
...
exports.getModerationStats = moderationRuntime.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    try {
        const stats = await admin_1.admin.firestore()
            .collection('content_moderations')
            .where('userId', '==', context.auth.uid)
            .get();
