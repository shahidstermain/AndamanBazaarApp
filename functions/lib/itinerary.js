...
const itineraryRuntime = functions.runWith({
  secrets: [secrets_1.SECRET_NAMES.GEMINI_API_KEY],
});
...
exports.generateItinerary = itineraryRuntime.https.onCall(
  async (data, context) => {
    if (!context.auth) {
...
Return only the JSON object.`;
    try {
      ...
      if (!validationResult.success) {
        ...
        throw new functions.https.HttpsError(
          "internal",
          "AI returned output with missing or invalid keys",
        );
      }
      ...
      for (const day of validatedData.days) {
        if (isNaN(new Date(day.date).getTime())) {
          throw new functions.https.HttpsError(
            "internal",
            `AI returned invalid date: ${day.date}`,
          );
        }
      }
      ...
      const itineraryRef = await admin_1.admin
        .firestore()
