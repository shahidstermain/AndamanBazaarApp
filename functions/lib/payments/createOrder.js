...
exports.createOrder = paymentsRuntime.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (duplicatePreventionKey) {
            const existingOrderQuery = await admin_1.admin.firestore()
                .collection('payments')
            ...
            if (!existingOrderQuery.empty) {
                ...
                v2_1.logger.info(`Duplicate order prevented: ${duplicatePreventionKey}`, {
        ...
        const cashfreeResponse = await (0, cashfreeClient_1.createCashfreeOrder)(cashfreeRequest);
        ...
        // Save to Firestore
        await admin_1.admin.firestore().collection('payments').doc(orderId).set(paymentOrder);
        ...
        await admin_1.admin.firestore().collection('listings').doc(listingId).update({
            ...
            reservationExpiresAt: admin_1.admin.firestore.Timestamp.fromDate(new Date(Date.now() + 15 * 60 * 1000)), // 15 minutes
        });
        ...
        if (data.listingId) {
            try {
                await admin_1.admin.firestore().collection('listings').doc(data.listingId).update({
                    status: 'active',
                ...
                v2_1.logger.error('Failed to cleanup listing reservation', cleanupError);
            }
        }
        if (error instanceof functions.https.HttpsError) {
            throw error;
        }
        throw new functions.https.HttpsError('internal', `Failed to create payment order: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
});
// Helper function to clean up expired reservations (can be called by scheduled function)
exports.cleanupExpiredReservations = paymentsRuntime.https.onRequest(async (req, res) => {
    try {
        const now = admin_1.admin.firestore.Timestamp.now();
        // Find expired reservations
        const expiredReservations = await admin_1.admin.firestore()
            .collection('listings')
        ...
        await batch.commit();
        v2_1.logger.info(`Cleaned up ${expiredReservations.size} expired reservations`);
        res.status(200).json({
            success: true,
