 ...
 */
exports.checkPaymentStatus = paymentsRuntime.https.onCall(
  async (data, context) => {
    ...
    try {
      ...
      const paymentDoc = await admin_1.admin
        .firestore()
        .collection("payments")
        .doc(orderId)
        .get();
      ...
      const cashfreeStatus = await (0, cashfreeClient_1.getCashfreeOrderStatus)(
        orderId,
      );
      ...
      if (needsUpdate) {
        ...
        v2_1.logger.info(
          `Payment status updated in Firestore: ${orderId}`,
          updateData,
        );
      }
      // Get the updated payment document
      const updatedPaymentDoc = await admin_1.admin
        .firestore()
        .collection("payments")
        .doc(orderId)
        .get();
...
async function handleStatusChange(currentPayment, newStatus) {
  ...
  if (
    ...
    currentPayment.paymentStatus !== "SUCCESS"
  ) {
    ...
    try {
      const listingRef = admin_1.admin
        .firestore()
        .collection("listings")
        .doc(currentPayment.listingId);
      ...
      if (listingDoc.exists && listingDoc.data().status !== "sold") {
        ...
        v2_1.logger.info(
          `Listing marked as sold via status check: ${currentPayment.listingId}`,
        );
      }
    } catch (listingError) {
  ...
  if (
    ...
    try {
      ...
      v2_1.logger.info(
        `Listing reservation released: ${currentPayment.listingId}`,
      );
    } catch (listingError) {
 ...
 */
exports.getPaymentHistory = paymentsRuntime.https.onCall(
  async (data, context) => {
    // Validate authentication
    if (!context.auth) {
      throw new functions.https.HttpsError(
 ...
 */
exports.getPaymentDetails = paymentsRuntime.https.onCall(
  async (data, context) => {
    // Validate authentication
    if (!context.auth) {
      throw new functions.https.HttpsError(
