 ...
 */
exports.createSeamlessOrder = paymentsRuntime.https.onCall(
  async (data, context) => {
    ...
    try {
      ...
      const orderRequest = {
        ...
        order_meta: {
          return_url:
            returnUrl ||
            `${process.env.FRONTEND_URL || "https://andamanbazaarfirebase.web.app"}/payment/success?order_id={order_id}`,
          notify_url:
            notifyUrl ||
            `${process.env.FUNCTIONS_URL || "https://api.andamanbazaar.in"}/cashfree/webhook`,
        },
        order_note: `Purchase of listing: ${listing.title}`,
      };
      v2_1.logger.info("Creating Cashfree order", {
        orderId,
        listingId,
        amount: orderAmount,
      });
      ...
      await admin_1.admin
        ...
        .set({
          ...
          expiresAt: orderData.order_expiry_time
            ? admin_1.admin.firestore.Timestamp.fromDate(
                new Date(orderData.order_expiry_time),
              )
            : admin_1.admin.firestore.Timestamp.fromDate(
      ...
      v2_1.logger.info("Payment order stored and listing reserved", {
        orderId,
        listingId,
      });
 ...
 */
exports.processSeamlessPayment = paymentsRuntime.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
    ...
    try {
      ...
      const ordersSnapshot = await admin_1.admin
        .firestore()
        .collection("payments")
      ...
      const response = await cashfree_pg_1.Cashfree.PGPayOrder(
        "2025-01-01",
        paymentRequest,
      );
      ...
      return {
        ...
        paymentStatus: "PENDING",
        paymentMessage:
          paymentData.action ?? "Payment initiated. Waiting for confirmation.",
      };
    } catch (error) {
 ...
 */
exports.getOrderPayments = paymentsRuntime.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
    ...
    try {
      ...
      const response = await cashfree_pg_1.Cashfree.PGOrderFetchPayments(
        "2025-01-01",
        orderId,
      );
      const payments = response.data || [];
      v2_1.logger.info("Payments retrieved", {
        orderId,
 ...
 */
exports.verifyOrderStatus = paymentsRuntime.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
