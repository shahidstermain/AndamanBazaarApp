...
exports.verifyOrderStatus = exports.getOrderPayments = exports.processSeamlessPayment = exports.createSeamlessOrder = void 0;
 ...
 */
exports.createSeamlessOrder = paymentsRuntime.https.onCall(async (data, context) => {
    // Verify authentication
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const { listingId, orderAmount, customerEmail, customerPhone, customerName, returnUrl, notifyUrl, } = data;
    // Validate required fields
    if (!listingId || !orderAmount || !customerEmail) {
        throw new functions.https.HttpsError('invalid-argument', 'Missing required fields: listingId, orderAmount, customerEmail');
    }
    try {
        initializeCashfree();
        // Verify listing exists and is available
        const listingDoc = await admin_1.admin.firestore().collection('listings').doc(listingId).get();
        ...
        v2_1.logger.info('Creating Cashfree order', { orderId, listingId, amount: orderAmount });
        const response = await cashfree_pg_1.Cashfree.PGCreateOrder('2025-01-01', orderRequest);
        const orderData = response.data;
        v2_1.logger.info('Cashfree order created successfully', {
            orderId: orderData.order_id,
            cfOrderId: orderData.cf_order_id,
            orderStatus: orderData.order_status,
        });
        // Store payment order in Firestore
        await admin_1.admin.firestore().collection('payments').doc(orderId).set({
            orderId: orderId,
            ...
            expiresAt: orderData.order_expiry_time
                ? admin_1.admin.firestore.Timestamp.fromDate(new Date(orderData.order_expiry_time))
                : admin_1.admin.firestore.Timestamp.fromDate(new Date(Date.now() + 15 * 60 * 1000)), // 15 min default
        });
        // Reserve the listing temporarily
        await admin_1.admin.firestore().collection('listings').doc(listingId).update({
            reservedBy: context.auth.uid,
            reservedAt: admin_1.admin.firestore.Timestamp.now(),
            reservationExpiresAt: admin_1.admin.firestore.Timestamp.fromDate(new Date(Date.now() + 15 * 60 * 1000)),
            updatedAt: admin_1.admin.firestore.Timestamp.now(),
        });
        v2_1.logger.info('Payment order stored and listing reserved', { orderId, listingId });
        ...
        v2_1.logger.error('Failed to create seamless order', {
        ...
        throw new functions.https.HttpsError('internal', `Order creation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
});
/**
 ...
 */
exports.processSeamlessPayment = paymentsRuntime.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const { paymentSessionId, paymentMethod } = data;
    if (!paymentSessionId || !paymentMethod) {
        throw new functions.https.HttpsError('invalid-argument', 'Missing required fields: paymentSessionId, paymentMethod');
    }
    try {
        initializeCashfree();
        // Find the order by payment session ID
        const ordersSnapshot = await admin_1.admin.firestore()
            .collection('payments')
        ...
        v2_1.logger.info('Processing seamless payment', {
        ...
        const response = await cashfree_pg_1.Cashfree.PGPayOrder('2025-01-01', paymentRequest);
        ...
        v2_1.logger.info('Payment initiated via PGPayOrder', {
            orderId: order.orderId,
        ...
        await admin_1.admin.firestore().collection('payments').doc(order.orderId).update({
        ...
        v2_1.logger.error('Failed to process seamless payment', {
            error: error instanceof Error ? error.message : 'Unknown error',
            paymentSessionId,
        });
        throw new functions.https.HttpsError('internal', `Payment processing failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
});
/**
 ...
 */
exports.getOrderPayments = paymentsRuntime.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const { orderId } = data;
    ...
    try {
        ...
        const orderDoc = await admin_1.admin.firestore().collection('payments').doc(orderId).get();
        if (!orderDoc.exists) {
            throw new functions.https.HttpsError('not-found', 'Order not found');
        }
        const order = orderDoc.data();
        ...
        v2_1.logger.info('Fetching payments for order', { orderId });
        // Fetch payments from Cashfree
        const response = await cashfree_pg_1.Cashfree.PGOrderFetchPayments('2025-01-01', orderId);
        const payments = response.data || [];
        v2_1.logger.info('Payments retrieved', { orderId, paymentCount: payments.length });
        // Update local order status based on latest payment
        if (payments.length > 0) {
            const latestPayment = payments[0];
            await admin_1.admin.firestore().collection('payments').doc(orderId).update({
        ...
        v2_1.logger.error('Failed to get order payments', {
            error: error instanceof Error ? error.message : 'Unknown error',
            orderId,
        });
        throw new functions.https.HttpsError('internal', `Failed to retrieve payments: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
});
/**
 * Verify Order Status
 ...
 */
exports.verifyOrderStatus = paymentsRuntime.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const { orderId } = data;
    ...
    try {
        ...
        v2_1.logger.info('Verifying order status', { orderId });
        // Fetch order details from Cashfree
        const response = await cashfree_pg_1.Cashfree.PGFetchOrder('2025-01-01', orderId);
        const orderData = response.data;
        v2_1.logger.info('Order status retrieved', {
            orderId,
            orderStatus: orderData.order_status,
        });
        ...
        v2_1.logger.error('Failed to verify order status', {
            error: error instanceof Error ? error.message : 'Unknown error',
            orderId,
        });
        throw new functions.https.HttpsError('internal', `Order verification failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
});
//# sourceMappingURL=seamlessPayment.js.map