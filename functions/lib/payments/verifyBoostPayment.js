 ...
 */
exports.verifyBoostPayment = paymentsRuntime.https.onRequest(async (req, res) => {
    // CORS
    ...
    try {
        ...
        if (boostSnap.empty) {
            ...
            const byCfId = await admin_1.admin.firestore()
                .collection('listingBoosts')
...
async function processVerification(boostDoc, uid, res) {
    ...
    try {
        const cfStatus = await (0, cashfreeClient_1.getCashfreeOrderStatus)(boost.orderId);
        ...
        if (newStatus !== boost.status) {
            ...
            await admin_1.admin.firestore().collection('listingBoosts').doc(boostDoc.id).update(updateData);
            // Audit log
...
async function generateBoostInvoice(boostId, boost, paymentInfo) {
    try {
        ...
        const invoiceDoc = {
            ...
            boostExpiresAt: admin_1.admin.firestore.Timestamp.fromDate(new Date(Date.now() + boost.durationDays * 24 * 60 * 60 * 1000)),
            // Status
            status: 'paid',
        ...
        await admin_1.admin.firestore().collection('listingBoosts').doc(boostId).update({
            invoiceId,
        });
        v2_1.logger.info('Invoice generated', { invoiceId, boostId, orderId: boost.orderId });
        return invoiceId;
    }
    catch (error) {
