...
exports.cashfreeWebhook = paymentsRuntime.https.onRequest(async (req, res) => {
  ...
  const isValidSignature = (0, cashfreeClient_1.verifyWebhookSignature)(
    payload,
    signature,
    undefined,
    timestamp,
  ...
  try {
    ...
    const existingEventQuery = await admin_1.admin
      .firestore()
...
async function processWebhookEvent(event, eventId) {
  ...
  switch (event.type) {
    ...
    case "ORDER_PAID":
      ...
      const purchaseRef = admin_1.admin
        .firestore()
    ...
    case "ORDER_FAILED":
      ...
      const listingRefFailed = admin_1.admin
        .firestore()
        .collection("listings")
        .doc(payment.listingId);
    ...
    case "ORDER_CANCELLED":
      ...
      // Release listing reservation
      const listingRefCancelled = admin_1.admin
        .firestore()
        .collection("listings")
        .doc(payment.listingId);
    ...
    case "ORDER_EXPIRED":
      ...
      paymentUpdate.paymentStatus = "CANCELLED";
      // Release listing reservation
      const listingRefExpired = admin_1.admin
        .firestore()
        .collection("listings")
        .doc(payment.listingId);
  ...
  const paymentRef = admin_1.admin
    .firestore()
    .collection("payments")
    .doc(orderId);
 ...
 */
exports.webhookHealthCheck = paymentsRuntime.https.onRequest(
  async (req, res) => {
    try {
...
async function processBoostWebhookEvent(event, boostDoc, eventId) {
  ...
  switch (event.type) {
    ...
    case "ORDER_PAID": {
      ...
      v2_1.logger.info("Boost activated via webhook", {
        boostId,
    ...
    case "ORDER_EXPIRED":
      ...
      break;
    default:
      v2_1.logger.info(`Unhandled boost webhook event type: ${event.type}`, {
        boostId,
        eventId,
      });
      return;
