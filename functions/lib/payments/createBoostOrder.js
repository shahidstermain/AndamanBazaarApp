...
exports.createBoostOrder = paymentsRuntime.https.onRequest(async (req, res) => {
    ...
    try {
        ...
        // ── Validate listing ────────────────────────────────────
        const listingDoc = await admin_1.admin.firestore().collection('listings').doc(listing_id).get();
        ...
        if (listing.userId !== uid) {
            v2_1.logger.warn('Non-owner attempted to boost listing', { uid, listingOwner: listing.userId, listing_id });
        ...
        const recentOrdersSnap = await admin_1.admin.firestore()
            .collection('listingBoosts')
            .where('userId', '==', uid)
            .where('createdAt', '>=', admin_1.admin.firestore.Timestamp.fromDate(oneHourAgo))
            .get();
        if (recentOrdersSnap.size >= 5) {
            v2_1.logger.warn('Rate limit hit on boost orders', { uid, count: recentOrdersSnap.size });
        ...
        const activeBoostsGlobalSnap = await admin_1.admin.firestore()
            .collection('listingBoosts')
            .where('userId', '==', uid)
            .where('status', '==', 'paid')
        ...
        const recentFailedSnap = await admin_1.admin.firestore()
            .collection('listingBoosts')
            .where('listingId', '==', listing_id)
            .where('userId', '==', uid)
            .where('status', 'in', ['failed', 'expired'])
            .where('updatedAt', '>=', admin_1.admin.firestore.Timestamp.fromDate(oneDayAgo))
            .limit(1)
            .get();
        ...
        const existingBoostSnap = await admin_1.admin.firestore()
            .collection('listingBoosts')
            .where('listingId', '==', listing_id)
            .where('userId', '==', uid)
        ...
        if (!existingBoostSnap.empty) {
            ...
            v2_1.logger.info('Returning existing pending boost order', { boostId: existing.boostId });
            res.status(200).json({
                success: true,
        ...
        const activeBoostSnap = await admin_1.admin.firestore()
            .collection('listingBoosts')
            .where('listingId', '==', listing_id)
            .where('status', '==', 'paid')
            .where('boostExpiresAt', '>', admin_1.admin.firestore.Timestamp.now())
        ...
        const boostId = admin_1.admin.firestore().collection('listingBoosts').doc().id;
        // ── Build return URL ────────────────────────────────────
        ...
        const cfResponse = await (0, cashfreeClient_1.createCashfreeOrder)(cashfreeReq);
        ...
        await admin_1.admin.firestore().collection('listingBoosts').doc(boostId).set(boostDoc);
        // ── Audit log ───────────────────────────────────────────
        ...
        // Store payment link
        await admin_1.admin.firestore().collection('listingBoosts').doc(boostId).update({ paymentLink });
        res.status(200).json({
            success: true,
