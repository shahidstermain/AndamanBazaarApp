...
exports.cashfreeWebhook = (0, https_1.onRequest)(async (req, res) => {
  ...
  try {
    ...
    try {
      cashfree_pg_1.Cashfree.PGVerifyWebhookSignature(
        signature,
        rawBody,
        timestamp,
      );
    ...
    if (
      ...
      if (isBoost) {
        ...
        try {
          const invoiceResult = await (0,
          generateInvoice_1.processInvoiceGeneration)(boostDoc.id);
          if (invoiceResult.success && invoiceResult.invoice_id) {
            await (0, sendInvoiceEmail_1.processSendInvoiceEmail)(
