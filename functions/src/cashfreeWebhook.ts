...
import { processInvoiceGeneration } from "./generateInvoice";
import { processSendInvoiceEmail } from "./sendInvoiceEmail";
      ...
      if (isBoost) {
        ...
        try {
          ...
          if (invoiceResult.success && invoiceResult.invoice_id) {
            await processSendInvoiceEmail(invoiceResult.invoice_id);
          }
        } catch (invoiceError) {
