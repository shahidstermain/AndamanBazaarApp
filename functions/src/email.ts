...
const sendViaResend = async (config: EmailServiceConfig, message: SendEmailRequest): Promise<void> => {
...
const sendViaSendGrid = async (config: EmailServiceConfig, message: SendEmailRequest): Promise<void> => {
...
const verifyAuth = async (req: functions.https.Request): Promise<admin.auth.DecodedIdToken> => {
...
export const sendEmail = functions.runWith({ secrets: ['EMAIL_API_KEY'] }).https.onRequest(async (req, res) => {
...
export const sendEmailInternal = async (to: string, subject: string, html: string): Promise<boolean> => {
  try {
    const emailConfig = getEmailConfig();
...
export const sendWeeklyTrendingEmails = functions.runWith({ secrets: ['EMAIL_API_KEY'] }).pubsub
