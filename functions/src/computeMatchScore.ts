 ...
 */
export const computeMatchScore = onCall(async (request: CallableRequest<any>) => {
  // Input Validation
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "User must be authenticated to compute personalized scores.");
  }
  
  if (!request.data.activityId || !request.data.price) {
    throw new HttpsError("invalid-argument", "Activity ID and basic attributes are required.");
  }
  ...
  try {
    ...
    if (!userPrefs) {
      return { matchScore: Math.round((activity.rating / 5) * 100), matchReasons: [] };
    }
    ...
    if (activity.price <= userPrefs.budget) {
      score += 30;
      if (activity.price >= userPrefs.budget * 0.7) reasons.push("Perfectly fits your budget");
    } else {
      const budgetFactor = Math.max(0, 1 - (activity.price - userPrefs.budget) / userPrefs.budget);
    ...
    } else if (userPrefs.groupType === 'solo' && activity.difficulty === 'Hard') {
      score += 5;
    }
