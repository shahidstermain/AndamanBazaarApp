 ...
 */
export const createBoostOrder = paymentsRuntime.https.onRequest(
  async (req, res): Promise<void> => {
    ...
    try {
      ...
      if (listing.userId !== uid) {
        logger.warn("Non-owner attempted to boost listing", {
          uid,
      ...
      if (recentOrdersSnap.size >= 5) {
        logger.warn("Rate limit hit on boost orders", {
          uid,
      ...
      const activeBoostsGlobalSnap = await admin
        .firestore()
        .collection("listingBoosts")
        .where("userId", "==", uid)
        .where("status", "==", "paid")
      ...
      const recentFailedSnap = await admin
        .firestore()
        .collection("listingBoosts")
      ...
      const existingBoostSnap = await admin
        .firestore()
        .collection("listingBoosts")
        .where("listingId", "==", listing_id)
        .where("userId", "==", uid)
      ...
      if (!existingBoostSnap.empty) {
        ...
        logger.info("Returning existing pending boost order", {
      ...
      const activeBoostSnap = await admin
        .firestore()
        .collection("listingBoosts")
        .where("listingId", "==", listing_id)
        .where("status", "==", "paid")
      ...
      // ── Build return URL ────────────────────────────────────
      const frontendUrl =
        process.env.FRONTEND_URL || "https://andamanbazaar.in";
