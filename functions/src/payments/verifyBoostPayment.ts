 ...
 */
export const verifyBoostPayment = paymentsRuntime.https.onRequest(async (req, res): Promise<void> => {
  // CORS
  ...
  try {
    ...
    if (boostSnap.empty) {
      ...
      const byCfId = await admin.firestore()
        .collection('listingBoosts')
...
async function processVerification(
  ...
  if (boost.userId !== uid) {
    logger.warn('Unauthorized verify attempt', { uid, boostOwner: boost.userId });
  ...
  try {
    ...
    if (cfStatus.orderStatus === 'PAID' || latestPayment?.paymentStatus === 'SUCCESS') {
      newStatus = 'paid';
    ...
    if (newStatus !== boost.status) {
      ...
      if (newStatus === 'failed') {
        updateData.failureReason = latestPayment?.paymentStatus || 'Payment failed at gateway';
      }

      await admin.firestore().collection('listingBoosts').doc(boostDoc.id).update(updateData);
...
async function generateBoostInvoice(
  ...
  try {
    ...
    try {
      const listingDoc = await admin.firestore().collection('listings').doc(boost.listingId).get();
    ...
    let userPhone = '';
    try {
      const userDoc = await admin.firestore().collection('users').doc(boost.userId).get();
      if (userDoc.exists) {
    ...
    await admin.firestore().collection('invoices').doc(invoiceId).set(invoiceDoc);
    ...
    logger.info('Invoice generated', { invoiceId, boostId, orderId: boost.orderId });
